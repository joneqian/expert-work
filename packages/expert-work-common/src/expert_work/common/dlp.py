...
Output screening (:mod:`expert_work.common.output_screen`) *blocks* a response
...
The patterns mirror ``expert_work.runtime.audit.redactor.PII_PATTERNS`` in
intent but are kept self-contained here: ``expert-work-common`` must not depend on
``expert-work-runtime`` (wrong layer direction). They are heuristics, not NER — names
