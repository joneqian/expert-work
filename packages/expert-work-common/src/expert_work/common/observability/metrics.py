...
1. **Naming-rule enforcement** at construction time. ``validate_metric_name``
   refuses anything that doesn't start with ``expert_work_``; ``validate_label_names``
   refuses high-cardinality labels (``session_id`` / ``trace_id`` /
   ``request_id`` etc.) that would blow up Prometheus storage.
2. **Convenience wrappers** — ``expert_work_counter`` / ``expert_work_histogram`` /
   ``expert_work_gauge`` — that run the validators upfront and then defer to
...
# § 5.2 + § 3.1: every Expert Work metric starts with ``expert_work_``. Mirror enums /
# values from the design doc into Python so the lint catches drift.
_METRIC_NAME_PATTERN: Final[re.Pattern[str]] = re.compile(r"^expert_work_[a-z][a-z0-9_]*$")
...
def validate_metric_name(name: str) -> None:
    ...
    - Must start with ``expert_work_``
    - Lower-case alphanumerics + underscores only
    - Histograms (see :func:`expert_work_histogram`) additionally need the
...
def expert_work_histogram(
    ...
    *,
    ...
    § 7: ``expert_work_*_duration_seconds`` / ``expert_work_*_latency_seconds``).
    Anything that should be a histogram in Expert Work records a duration, and
