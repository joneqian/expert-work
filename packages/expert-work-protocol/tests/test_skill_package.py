...
def test_parse_minimal_frontmatter() -> None:
    """Standard external frontmatter (name + description only, no expert_work:
    namespace — the Anthropic/Vercel format imported via GitHub) must parse.
    ``expert_work.version`` is expert-work-internal and defaults to 1 when absent; the DB
...
"""
    ...
    assert parsed.expert_work_version == 1  # default, not a reject
    assert parsed.expert_work_category is None
    assert parsed.expert_work_required_models == ()
    assert parsed.expert_work_tool_names == ()
    assert parsed.expert_work_authored_by == "human"
    assert parsed.body == "# Body\nhello"


def test_parse_expert_work_version_zero_rejected() -> None:
    """When expert_work.version IS present it is still type/range-checked."""
    text = """---
name: x
description: y
expert_work:
...
body
"""
    with pytest.raises(SkillPackageLayoutError, match=r"expert_work.version"):
        parse_skill_md(text)


def test_parse_with_expert_work_namespace() -> None:
    text = """---
name: api-debug
...
"""
    ...
    assert parsed.expert_work_version == 2
    assert parsed.expert_work_category == "ops"
    assert parsed.expert_work_required_models == ("anthropic/claude-sonnet-4",)
    assert parsed.expert_work_tool_names == ("http", "exec_python")
    assert parsed.expert_work_authored_by == "human"
    assert parsed.expert_work_lazy is False
...
def test_parse_expert_work_lazy_must_be_bool() -> None:
    text = """---
name: x
description: y
expert_work:
...
def test_parse_expert_work_authored_by_only_human_or_agent() -> None:
    text = """---
name: x
description: y
expert_work:
...
def test_serialize_omits_expert_work_defaults_for_clean_diff() -> None:
...
def test_parse_omitted_lazy_defaults_to_progressive_disclosure() -> None:
    # RT-ADR-11 — frontmatter without a ``expert_work.lazy`` key (the common GitHub
...
"""
    ...
    assert parsed.expert_work_lazy is True
...
def test_serialize_round_trips_lazy_flag(lazy: bool) -> None:
    ...
    assert reparsed.expert_work_lazy is lazy
