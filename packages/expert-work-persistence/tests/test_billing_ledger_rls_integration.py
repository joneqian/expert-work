...
bypass every policy, so we provision a fresh non-superuser ``expert_work_app``
LOGIN role, GRANT it CRUD on the schema, and ``GRANT audit_reader TO
expert_work_app`` so it can ``SET ROLE`` to the BYPASSRLS reader — exactly how
...
from expert_work.persistence.billing.ledger import DbTenantBillingLedgerStore
...
APP_ROLE = "expert_work_app_billing_z2"
APP_PASSWORD = "expert_work_app_billing_z2_pw"  # test-only fixture password
