...
> **背景**：2026-06-10 完成 Expert Work harness 十维能力评估（`docs/research/2026-06-10-expert-work-harness-capability-assessment.md`），用户拍板把全部非强项做强，按 Wave 严格顺序推进（Wave 1 零债收完再进 Wave 2）。每条子项设计先行 + 零债收尾（Stream CM 同节奏）。
>
...
| 估算器 B：`default_token_estimator` = `len(str(content)) // 4`，独立重复实现 | `expert-work-runtime/.../middleware/dynamic_context.py:26-38` | 弱 + 重复 |
...
| `tools/` 与 control-plane 无 `estimate_tokens`/`context_window` 消费 | 全仓 grep | sweep 面收敛在 orchestrator + expert-work-runtime + protocol |
...
**新模块** `packages/expert-work-runtime/src/expert_work/runtime/tokens.py`（runtime 包：middleware 在此、orchestrator 已依赖 runtime，依赖方向成立）：
...
3. `expert-work-runtime` pyproject 增 `tiktoken>=0.8,<1`（与 control-plane 同约束；注意 uv.lock 漂移）。

**可观测（零债项）**：`TokenUsageMiddleware`（after_llm_call，usage_metadata 真值已在手，`prompt_messages` 已在 payload）增 estimator 注入，发 `expert_work_ew_token_estimated_total` counter（估算 prompt token 累计；本地 cache hit 跳过）。漂移比在 PromQL 侧求：`rate(expert_work_ew_token_estimated_total) / rate(expert_work_llm_token_usage_total{type=~"input|cache_.*"})`——既是 HX-1 的验收数字（上线后看比值是否从 ~0.4（CJK 低估）收敛到 ~1.0），也是 §2.2-E 未来校准的数据源，并入 HX-4 指标族。**实施期修正**：原设计为 ratio histogram，仓内指标公约（`expert_work_histogram` 强制 `_seconds` 后缀）保留直方图给时长类，改 counter 对零公约破坏。
...
- **HX-A6 drift 可观测**：`expert_work_ew_token_estimated_total` counter（估算 prompt token 累计）进 TokenUsageMiddleware，与既有 `expert_work_llm_token_usage_total{type=input/cache_*}` 真值在 PromQL 求漂移比；验收数字 + 未来校准数据源，归 HX-4 指标族。（实施期修正：原 ratio histogram 违反"直方图仅时长"公约，改 counter 对。）
...
- `expert_work_control_plane_feedback_consumed_total{action=memory_flagged/noop}` counter + worker cycle error counter（既有命名纪律）。
...
- env：`EXPERT_WORK_RUN_TRANSIENT_RETRY`（默认 on）、`EXPERT_WORK_RUN_RETRY_BACKOFF_S`（默认 10，clamp [1, 120]，解析失败回默认不 raise）。重试次数固定 1（常量不开配置面——多次重试的退避策略面是另一个需求）。
...
- `expert_work_orchestrator_run_retry_total{outcome="recovered"|"failed_again"}` counter（recovered = 重试后到达 SUCCESS/PAUSED；failed_again = 二轮仍失败）。守卫拒绝不发 counter（拒绝即走既有 ERROR 路径，error 字段已可观测）——但 log 一条 `run_retry.guard_rejected`（含 dangling 批次工具名）。
...
| 评估断言 | 实况 | 证据 | 判定 |
|---------|------|------|:----:|
| "工具延迟直方图缺" | `expert_work_tool_latency_seconds{tool}`（桶 0.01-60s）+ `expert_work_tool_call_total{tool,outcome}` 已在，TOOL_CALL audit 还带 `duration_ms` | `builder.py:142-153,1087-1098,1139` | **取证修正 ①：已在** |
| "run 成功率 counter 缺" | `expert_work_session_duration_seconds{outcome}` 五种 outcome（success/error/max_steps/interrupted/cancelled）每终态恰好一次 observe——histogram `_count` 即按 outcome 的 run 计数，成功率 PromQL 直接推导 | `sse.py:116-121,577` | **取证修正 ②：可推导，不加冗余 counter**（缺的是 recording rule） |
...
gauge helper 已在（`expert_work_gauge`，`metrics.py:121-132`）且有先例：skill_curator 周期 worker 每 cycle `set_curator_pinned_skills(n)`（`skill_curator.py:195`）。
...
- control-plane lifespan 内轻量周期任务（curator 同款骨架，interval 60s，复用现有 worker 节奏不新开配置面）：`expert_work_control_plane_approvals_pending` gauge `.set(n)`。读失败 log + 跳过本 cycle（fail-open：可观测故障绝不影响业务路径）。

**② checkpoint 计时 wrapper（gap ②）**

- `expert-work-runtime/checkpointer/timing.py`：`TimingCheckpointSaver` 代理 `BaseCheckpointSaver` 四个 IO 方法（`aput`/`aput_writes`/`aget_tuple`/`alist`），`expert_work_checkpoint_op_seconds{op}` histogram（桶对齐 0.005-10s 量级；`alist` 流式则计首批返回时延或不计——实施期按基类签名定）。计时层 never-fail：观测代码异常吞掉照常透传调用（fail-open）。
- `make_checkpointer` 统一包两种实现（backend 不进 label——dev InMemory 数据没人看，统一包保测试路径同形）。`GraphRunner` 零改动。

**③ run_id 结构化贯穿（gap ③ 半轴）**

- `common/context.py` 增 `current_run_id` contextvar（trace_id 同款三函数面）；`ExpertWorkJsonFormatter` 输出增 `run_id` 字段（未设 = null，与 trace_id 同语义）。
...
- `tools/observability/rules/sli.yml` 增：run 成功率（`sum(rate(expert_work_session_duration_seconds_count{outcome="success"})) / sum(rate(..._count))`）、HX-3 retry 恢复率、HX-1 token 估算漂移比、approvals pending、checkpoint op P95。
...
| `expert_work_eval.run_eval(eval_set, complete)` 轻量 harness 在：YAML EvalSet（cases+assertions）→ per-case 判定 → EvalReport；judge 是断言式非 LLM | `tools/eval/expert_work_eval.py:145-157`、`datasets/example.yaml` | 离线对比的三件套有两件（任务集+运行器），缺 variant 对比层 |
...
- `tools/eval/prompt_ab.py` CLI：输入 = agent spec 文件（或 name+revision 两个引用）×2 + EvalSet YAML + provider 配置；对每个 variant 用其 system_prompt 组装 CompletionFn（复用 expert_work_eval 既有 provider 适配），跑同一 eval set，输出对比报告（per-case A/B passed 矩阵 + 通过率 Δ + McNemar 不对称计数——n 小用精确二项，不编 p 值阈值，报数让人判断）。
...
- 不做 LLM judge（expert_work_eval 是断言式；M1 J.13a-2 既定项，另立）。
...
- `expert_work_sandbox_pool_ready` gauge（per variant）+ `expert_work_sandbox_pool_total{event=hit/miss/replenish/replenish_failed/claim_raced}` counter。
- `expert_work_sandbox_cold_start_seconds` 语义不变（池命中不计入，同 J.15 暖路径）；SLO #4 M1 行的验收数字 = 池命中率 + 命中路径延迟（claim+update 应 <500ms）。
...
| 评估断言 | 实况 | 证据 | 判定 |
|---------|------|------|:----:|
| "trace 无生产后端" | **OTel 链已全通**：W3C traceparent 提取 → contextvar → `X-Expert-Work-Trace-Id` 回送 + 日志贯穿（HX-4 又加 run_id 轴）；`otlp_traces_endpoint` settings 已在 | `middleware/observability.py:84-106`、`context.py:30-36`、`settings.py:66` | 半过期：基础设施 trace（Tempo 路线）只差部署；**真缺口 = Langfuse 档** |
...
- `LangfuseSdkClient` 实现既有 `LangfuseClient` Protocol，新模块 `packages/expert-work-runtime/src/expert_work/runtime/middleware/langfuse_sdk.py`；middleware 一行不动（缝就是 Protocol——Mini-ADR HX-G1）。
- 依赖：`langfuse>=3,<4` 进 expert-work-runtime（v3 SDK 基于 OTel，与现有 `opentelemetry-sdk>=1.27` 同源——Langfuse span 自动挂进活跃 OTel trace，ADR-0005 "trace_id 共享、Langfuse↔Tempo 互跳" 数据流**零额外代码兑现**）。import 失败防御降级 Recording（损坏安装不杀服务）。
...
- control-plane settings 增 `langfuse_host` / `langfuse_public_key` / `langfuse_secret_key`（env `EXPERT_WORK_CONTROL_PLANE_LANGFUSE_*`；secret 仅 env 注入，不进代码/compose 明文——object_store keys 先例）。
...
- **HX-G2 `langfuse` 依赖进 expert-work-runtime + import 防御降级**：observability 是平台默认能力，不做 extra 可选安装位；损坏的 SDK 安装降级 Recording 而非启动失败。
...
2. **resolver 的租户接缝已在签名里**：`CredentialsResolver.resolve_provider/resolve_tool`（`expert-work-common/credentials/resolver.py:101`）已收 `tenant_id` 参数，但现仅做租户存在性验证（Y-1）——凭证查询 100% 平台级。embedder/reranker/web_search/agent 主模型 key 全走此路径（`control_plane/runtime.py:455-618`）。
...
> **实施修订（PR1，2026-06-12）**：`expert-work-common/credentials` 路径为 harness 禁写区，"resolver 加可选 kwargs" 不可实施。等价替代：control-plane 新模块 `tenant_secret_overlay.py` 的 **`TenantOverlayCredentialsResolver(CredentialsResolver)` 子类**——override `resolve_provider/resolve_tool` 直查 tenant-effective 视图（错误契约与基类 platform 路径逐字段一致：`mode="platform"` + kind + key），`app.py` 构造点换子类。expert-work-common 零接触，合并逻辑仍全收 service——HX-H3 语义不变且更纯。
...
- settings：`cross_tenant_query_enabled: bool = True`（`EXPERT_WORK_CONTROL_PLANE_CROSS_TENANT_QUERY_ENABLED`，default 保现状）。
...
- `expert_work_platform_credentials_tenant_overrides` gauge（service `_reload()` 时 set，租户 override 总行数）——运营一眼看 override 面有多大。
...
6. **HX-1 资产可用**：`TokenEstimator` 协议 + `TiktokenEstimator`（`expert-work-runtime/tokens.py`，`default_estimator()` 单例）已落地；`context_window` 解析（manifest override → MODEL_CATALOG）在 agent_factory。阈值逃生门可直接用真 tokenizer（ITERATION-PLAN 原文"chars/4，HX-1 落地后换"已过期——直接上真分词）。
7. **jieba 已是仓内依赖**（expert-work-persistence，J.5 `knowledge/text_search.py` app-side CJK 分词先例）；无 BM25 实现/依赖。
8. **指标**：`expert_work_tool_call_total{tool,outcome}` + `expert_work_tool_latency_seconds{tool}` 已在（builder.py:143）；promotion 域零指标。
...
- 零命中打 `expert_work_tool_promotion_total{event="miss"}`（治理信号，将来喂 HX-2 学习闭环）。
...
- dispatch 成功路径检测：名字**在 deferred 池** → 执行照常（本就 dispatchable）+ **补写 promote**（outcome 的 state_updates 合并 `promoted_tools`，重复 promote 由 reducer dedupe 幂等）+ `expert_work_tool_promotion_total{event="call_through"}`——模型记得工具名就不必付 find_tools 往返，且 schema 进入后续 bind 不再盲调。
...
- 指标：`expert_work_tool_promotion_total{event}`，event ∈ {promote, call_through, demote, miss}；现有 `expert_work_tool_call_total` 不动。
...
- `expert_work_tool_promotion_total{event=promote|call_through|demote|miss}`（10.2.5）。
- 既有 `expert_work_tool_call_total{tool,outcome}` / `expert_work_tool_latency_seconds` 不动；call-through 的执行打点走既有 outcome 维度。
...
- `expert_work_tool_promotion_total` 四事件继续工作（native 档 call-through 即服务端检索回流的 promote 信号）。
- 降级事件：`expert_work_llm_tool_disclosure_fallback_total{provider}` counter + WARNING 日志（一次性）。
...
- **settings**：`seccomp_profile_path: str | None = None`，env `EXPERT_WORK_SANDBOX_SECCOMP_PROFILE_PATH`；默认 None 保 dev/单测零变更，environments yaml 显式开。docker-out-of-docker：路径是**宿主可见路径**（compose volume / 宿主预置），不是 supervisor 容器内路径。

#### 12.2.2 ② gVisor 可配置 + CI 验证（PR2）

实证消除了缓做理由（代价小 + 国内云可跑 + 关闭 40%-可利用残差），故当期纳入，形态 = **可配置 + CI 验证**，非全局无脑强制。

- **部署策略（写入 environments 文档）**：生产多租户同宿主**推荐 runsc**（`EXPERT_WORK_SANDBOX_OCI_RUNTIME=runsc`）；dev/macOS runc（gVisor Linux-only）。能力代码（runc↔runsc）本就在，本 PR 只补**验证**。
...
- **集成测试 runtime 参数化**（PR2 配套）：4 处硬编码 `oci_runtime="runc"` → 读 env `EXPERT_WORK_TEST_SANDBOX_RUNTIME`（默认 runc，本地/既有 job 零变更）；gVisor workflow 设 runsc。
...
**落地**：
- **接缝（#592 已交付）**：`runtime_provider.extra_hosts`→`--add-host hostname:ip` argv（顺序保持）+ `EXPERT_WORK_SANDBOX_EXTRA_HOSTS`（`name=ip,…`，格式错启动期 fail-closed 同 seccomp）+ supervisor app 接线（`make_sandbox_runtime_provider(extra_hosts=parsed_extra_hosts)`）。
...
- **compose 落默认**：`expert-work-sandbox-egress` 加 `ipam` subnet + credential-proxy 钉 `ipv4_address: 172.30.0.10` + supervisor 注释给出 runsc 下应配的 `EXPERT_WORK_SANDBOX_EXTRA_HOSTS` 具体值。dev/runc 仍留 unset（embedded DNS 正常）。
- **剩纯运维**：生产选不撞的私网段（避开 VPN/VPC/其他 docker 网络）+ runsc 环境配 `EXPERT_WORK_SANDBOX_EXTRA_HOSTS`。无代码债。
...
| SSRF | `expert-work-common/url_validation.py:44` / `api/mcp_servers.py:52` host-pivot 字符黑名单 | URL 注册双段校验 |
...
**事件 payload**：`{event_id, event_type, occurred_at, tenant_id, seq, payload}`；at-least-once，消费方按 `(tenant_id, seq)` 幂等。**签名**：HMAC-SHA256 per-endpoint secret（header `X-Expert-Work-Signature-256`），**请求绝不携带平台凭证**。**SSRF**：注册时 `validate_remote_url`（私网/metadata 阻断）+ host-pivot 字符黑名单；投递前解析后 IP 再校验（防 DNS rebinding）。
