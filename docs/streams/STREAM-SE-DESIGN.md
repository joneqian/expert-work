...
| SE-A44 | implicit_success 来源永不 auto-promote(输入弱标注→强制人审累积信任) | §12 SE-16 |
...
### 12.3 SE-A38 — 隐式正信号(P0)

`CurationWorker._classify` 对「无标记 + success」轨迹补一层纯规则判定(不用 LLM,零 aux 成本):

- 该 thread 无任何 👎;
- 轨迹结束后**无短时重发**(同 thread 下一条 user 轮距上一 assistant 轮 < 5 分钟视为改口/重试
  —— 数据源 = `thread_message` 镜像,#892 现成);
- run 结局 success。

三条全过 → 新 `CurationSignal.IMPLICIT_SUCCESS` 候选。进 `EVOLVE_SIGNALS`。

**SE-A44 证据对齐**:implicit 来源候选蒸馏产物即便重放 T1 pass,**auto-promote 强制降级人审**
(`decide_promotion` 加 origin_signal 维度)。理由:输入是弱标注,先经人审累积信任,数据够了再放开。
...
- rollback 跨时段对照的统计弱点:熔断兜底可接受;shadow-eval(留少量 without 流量)远期再议。
- 产物下载类隐式信号:无审计埋点(仅 quota 计数),先不做。
...
|---|---|---|
| D | 本设计 | — |
| 1 | SE-A40 重试档 + SE-A41 灰度(小而独立,先解锁 live 试点) | D |
| 2 | SE-A38/A39/A44 信号扩容(classify 升级 + EVOLVE_SIGNALS + failures 注入 + implicit 禁 auto) | 1 |
| 3 | SE-A42 auto_attach_evolved_skills(spec 字段 + build 装配 + 表单/i18n) | 1 |
| 4 | SE-A43 aux 计量 | 1 |
| 5 | promote 事件发射(触达渠道另立项) | 3 |

验证:各 PR 单测 + 真 PG 集成;收官标准 = **live 试点**(灰度开单租户,真实 👎/隐式成功驱动
一轮 蒸馏→人审→attach→监控 全链),CI 绿≠live 能跑(★5 纪律)。
