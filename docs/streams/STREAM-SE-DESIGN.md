...
设计先行规则([memory:feedback_design_first_iteration](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_design_first_iteration.md)):所有总体架构 / 跨切面接口 / Mini-ADR 必须在编码前于本文件锁定;每个子项 PR 只执行本文件对应章节 + 其局部细化设计。

> **对标纪律**:openclaw / deer-flow / hermes-agent 作能力基线,校准"成熟长什么样"+ 找差距。**结论是独立设计,不照抄**([memory:feedback_general_platform_positioning](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_general_platform_positioning.md))。三仓实证见 [docs/research/2026-06-06-self-evolving-skills.md](../research/2026-06-06-self-evolving-skills.md) § 10。

> **能力不可弱**([memory:feedback_complete_not_minimal](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_complete_not_minimal.md) / [no-design-choice-disguise](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_no_design_choice_disguise.md)):本 Stream 的咽喉是**验证(grounding)**。不允许把"只靠 LLM 自评 / 只靠 prompt 自觉"包装成设计选择 —— 那正是三仓(含当前 Expert Work)共同的缺口,也是学界三篇论文(CoEvoSkills / SPARK / EmbodiSkill)主攻处。

---

## 0. 为什么做(Context)

Expert Work 当前的 skill 是"**人写、版本化、静态启用**"(Stream J.7a + X):skill 由 admin 创作、走 draft→active 审核、agent 在 manifest 里按名引用。但 [目标产品形态](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/project_target_product_form.md) 是 per-user 持久 agent —— agent 应当**从自己的使用经验里持续长出能力**,而不是永远等人来写 skill。
...
3. Expert Work 独有服务端 trajectory + eval 基建,**天生适合 SPARK 式"后验蒸馏 + 重放验证"** —— 这是把别人做不到的"被证据验证过的自进化"做出来的机会。
...
## 2. 总体架构:自进化闭环映射到 Expert Work

闭环六环节(报告 § 3)与 Expert Work 组件的映射:
...
- **Layer B(后验蒸馏引擎,Expert Work 差异化)**:后台 worker 离线从真实轨迹自动产 skill(SE-6)。数据驱动、规模化。
...
| 能力 | 锚点 | 复用方式 |
|---|---|---|
| skill 协议/版本/高危/hash | `packages/expert-work-protocol/src/expert_work/protocol/skill.py`(`SkillVersion`、`is_high_risk_skill_version`、`compute_content_hash`、`HIGH_RISK_TOOLS`)| 加字段,不改语义 |
| skill 持久化 + curator | `packages/expert-work-persistence/.../persistence/skill/{base,sql,memory}.py`、`services/control-plane/src/control_plane/skill_curator.py`、迁移 0029/0042/0043/0057 | 加 store 方法 + 新迁移;curator 自动覆盖 agent_private skill |
...
| judge / eval | `tools/eval/_judge.py`(`AnthropicHaikuJudge`/`ScriptedJudge`)、`tools/eval/expert_work_eval.py`(`EvalCase`/`Assertion`/`run_eval`)| replay 打分复用 |
...
> 结论:Expert Work 已有 ~80% 地基。本 Stream 本质是**补三缺口(自著工具 / 后验蒸馏 / 重放验证)+ 串闭环 + 加全自动护栏**。
...
> 上面三问的方法,威力几乎都来自"环境自带 ground-truth 预言机"(Minecraft 物品 / 单测 / deterministic verifier)。但 Expert Work 头号形态是 per-user 持久 agent 做开放式业务/知识工作,**大面积任务无硬 verifier**。这片是 SE-4 最大的未验证风险,故补查一轮"无预言机下的可信验证"。
...
- **架构印证 —— SkillGen(2605.10999)**:把 skill 合成建模为 **intervention problem**(= Expert Work baseline-vs-treatment),pipeline = **contrastive induction → generation-verification-refinement loop → 按 held-out 净效应选 skill(计入修复与回归)**,产出单个**可审计** skill。这与 SE-4(净效应验证)+ SE-5(轨迹蒸馏)+ SE-6(gen-verify-refine 环)几乎重合 → 验证 Expert Work 路线是 SOTA。
...
- 内容安全复用:U-22 threat scan + content_hash drift + 沙箱。全程 AuditAction + Prometheus 指标(`expert_work_skill_evolution_*`)。
...
落 [admin UI 设计基线](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/project_admin_ui_design_baseline.md);接线点清单见 [memory:admin_ui_wiring_touchpoints](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/reference_admin_ui_wiring_touchpoints.md)。**不新建路由 / 不加侧导航项**:
...
样板 `api/{skills,curation,platform_skills,audit}.py`;envelope 对账见 [memory:envelope-vs-raw](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_envelope_vs_raw_contract_check.md)(skill 端点返 raw,SDK 用 `apiClient` 直取不 `getJson`)。
...
> [memory:no-design-choice-disguise](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_no_design_choice_disguise.md):不允许把弱护栏说成"够用"。
...
## 9. CI / 约束(复用 Expert Work 门禁)

- 迁移 0065 revision id ≤ 32 字符(0058 已占用、链头 0064)([memory:alembic_revision_id_32_chars](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_alembic_revision_id_32_chars.md));只在 integration 验证。
- `audit.py` 双 Literal(protocol+control-plane);mypy 不覆盖 control-plane/src([memory:ci_lint_type_test_scopes](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/reference_ci_lint_type_test_scopes.md));pytest `-m "not integration"`。
- CodeQL:审计 / 日志脱敏 —— 别 log 请求派生值([memory:codeql_log_injection_request_taint](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_codeql_log_injection_request_taint.md))、别 log secret 命名名字、assert 无副作用、Protocol 体用 docstring、不留未引用 module-level 名。
- 协议改签名 sweep 含 tools/eval doubles([memory:protocol_sweep_includes_tools_eval](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/reference_protocol_sweep_includes_tools_eval.md))。
- skill 表 RLS:**ENABLE-only 不能加 FORCE**([memory:skill_curator_owner_rls_exemption](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/reference_skill_curator_owner_rls_exemption.md)) —— evolution worker 跨租户 sweep 依赖 owner 豁免,加 FORCE 会静默停扫。
- 每 PR:push 前 `pre-commit run --all-files`([memory:uv_lock_and_precommit_ruff](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_uv_lock_and_precommit_ruff.md) / [ruff_strict_lint_traps](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_ruff_strict_lint_traps.md))+ 同步 ITERATION-PLAN(checkbox+PR#,[memory:iteration_plan_sync_after_ship](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_iteration_plan_sync_after_ship.md))+ 零技术债收尾([memory:feedback_zero_tech_debt](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/feedback_zero_tech_debt.md))。
...
Expert Work 是**声明式 harness**(`AgentSpec` manifest 装配成 `BuiltAgent`),非外部那种文件树式工作区。SE-15 把外部 HARNESS.md v1.0 的 7 组件**映射到 AgentSpec 字段**(规范 `docs/HARNESS-COMPLIANCE.md`),并补一个可执行校验器,覆盖 `model_validator` 表达不了的跨组件 / 命名 / 合规规则。
...
- **SE-A37 profile 复用思想不照搬工具**:`tools/harness/expert_work_profile.yaml`(对标外部 hermes/codex/openclaw profile 的「不同形态不同必填」思想);外部 `validate_harness.py`(文件树审计器)与 manifest 形态不符,**不进 CI**。
...
| SE-A38 | 隐式正信号 IMPLICIT_SUCCESS:纯规则判定(无👎+无短时重发+success),不用 LLM | §12 SE-16 |
| SE-A39 | 👎 变燃料:作 failures 语料(带 comment)喂对比蒸馏,不单独成候选;仅有👎无正例仍跳过 | §12 SE-16 |
| SE-A40 | 蒸馏 transient 异常不烧候选:retry_count 列,≥3 或永久错误才标 evolved_at | §12 SE-16 |
| SE-A41 | per-tenant 灰度:tenant_config `skill_evolution_enabled`(默认 false)与平台总闸两层与 | §12 SE-16 |
| SE-A42 | 晋升→生效补链:AgentSpec `auto_attach_evolved_skills` opt-in,不偷改 manifest | §12 SE-16 |
| SE-A43 | aux LLM 计量进 token_usage(真 tenant_id + 源 agent),进化成本可 chargeback | §12 SE-16 |
| SE-A44 | implicit / judge-scored 来源永不 auto-promote(弱标注→强制人审累积信任);👍 来源可 auto | §12 SE-16 |
| SE-A45 | judge 抽样评分=候选提纯(默认 5%,便宜模型),与晋升闸 judge 体系分离 | §12 SE-16 |
| SE-A46 | 打标=校准集+语料非主粮:Playground 👍/👎 补落地(端点 G.6 就绪但无调用方)+ 业务系统 API 文档化 | §12 SE-16 |
| SE-A47 | 蒸馏入库查重:embedding 相似→转既有技能修订轮,不新建;merge 不做 | §12 SE-16 |

---

## 12. SE-16 — 进化飞轮可运转性 uplift(2026-07-02 全链评估驱动)

> 触发:用户问「进化自建 Skill 完整逻辑」→ 全链只读评估。结论:**验证-晋升-回滚闭环成熟,
> 但飞轮不可运转** —— 输入端信号饥饿、输出端生效断链、生产端无灰度。本节记录核实结论
> 与修复设计;实施拆 5 个 PR(见 § 12.9)。

### 12.1 核实结论(2026-07-02,file:line 证据在评估记录)

| 疑点 | 结论 |
|---|---|
| 晋升闸是否会被 judge-only 证据穿透 | **否,已达标**:`grounding._auto_eligible` 三层 tier —— T1 HARD_VERIFIER 自动;T2 CALIBRATED_JUDGE 需 anchors_passed AND judge_calibrated;T3 UNVERIFIED 永不自动。无需收紧 |
| 技能注入是否全量膨胀 prompt | 否:manifest 显式列表 + eager fragment / summary 两档,`lazy_load` 已有。**但反向问题成立**:ACTIVE 后不自动进任何 agent(见 12.5) |
| rollback 对照组 | 线上 with 流量 vs **晋升时点 replay baseline**(跨时段,漂移可污染);熔断兜底。短期接受,局限记录在案(§ 12.8) |
| aux LLM 计量 | **不走 token_usage**;judge 用 `_NULL_TENANT=UUID(int=0)` 占位。平台垫付糊涂账(见 12.7) |

### 12.2 断点总图

```
信号 ──✂ 饥饿(成功路只认👍;👎收集了不用;transient失败烧候选)
  → 候选 → 蒸馏 → 重放验证 → 晋升闸(✓成熟)
                                → ACTIVE ──✂ 断链(无 agent 装配 → 无 with 流量
                                              → last_used_at 不 bump → 30/90 天被 curator 收走)
生产 ──✂ 无灰度(平台级 bool,全租户齐开齐关 → 默认 False 至今)
```

### 12.3 信号金字塔(P0)— SE-A38 / A44 / A45 / A46

> 业界对标(2026-07-02 调研):生产环境显式打标覆盖率 **<1%**(乐观 1-3%),且强负偏;
> 没有严肃玩家把打标当主信号。主流 = 三层金字塔:隐式行为信号(100% 覆盖)→
> LLM-as-judge 抽样评分(1-5% trace,observability 生态标配)→ 显式打标(稀疏校准集)。
> Agent 技能自进化论文群(Voyager 谱系 / Trace2Skill / SkillsVote / SkillAudit)更彻底:
> 全走轨迹自动信号,无一依赖用户打标。原设计把 👍 当成功路主信号 = 权重错,本节重排。

**底座 — SE-A38 隐式规则信号**(覆盖 100% 成功流量,零 aux 成本):
`CurationWorker._classify` 对「无标记 + success」轨迹补纯规则判定:

- 该 thread 无任何 👎;
- **thread 已静默收尾**(实施期修正,原「5 分钟重发」规则会把正常多轮追问全排除:快速
  接话是正常使用不是改口。改判 run 级信号——thread 无失败/待审 run 且末次 run 静默
  ≥30 分钟 =「用户拿到答案走了」,RunStore 聚合现成零新依赖;活跃 thread 只是
  「还不是候选」,下轮 sweep 重评,静默后自然入选);
- run 结局 success。

三条全过 → 新 `CurationSignal.IMPLICIT_SUCCESS` 候选。进 `EVOLVE_SIGNALS`。

**中层 — SE-A45 judge 抽样评分**(业界标配,恢复进本 wave):
对通过 SE-A38 规则的候选池,再抽样(`tenant_config` 可配,默认 5%)用**便宜 aux 模型**
批量评「会话质量分」(任务是否真完成、回答是否对题),高分才保留为候选。定位 =
**候选提纯**,不是晋升判定 —— 与晋升闸的 judge 校准体系(T2 tier)完全分离,不引入
晋升侧偏差。成本上限 = 抽样率 × 便宜模型,且计量走 SE-A43。
实施注记(PR-4 定):筛点在 **evolution worker 蒸馏前**而非 CurationWorker 检出时——
候选仍全量落库留审计,`implicit_success` 候选进蒸馏前过筛;抽样按 `trajectory_key`
哈希**确定性**分桶(transient 重试重扫不重掷,测试可复现);未抽中/低分(<4)标
`evolved_at` 永久出队(日志 + `screened_out` metric 可见),抽样率同时成为隐式池
蒸馏量的成本上限;judge transient 错误走 SE-A40 重试档;aux 调用记真 tenant_id。

**顶层 — SE-A46 显式打标 = 校准集 + 语料,不是主粮**:
- 核实结论:`POST /v1/sessions/{thread_id}/feedback` 端点自 G.6 就绪,但**全仓无任何调用方**
  ——👍/👎 按钮从未落地(Stream H 漏项),打标率现状 = 0。
- 补两个入口:**Playground 每条 assistant 轮挂 👍/👎**(👎 弹 comment 框)——内部验收/
  试点人群打标率远高于终端用户,是灰度试点的信号主力;**业务系统代提交**(末端 UI 归
  业务系统是既定形态),契约(201 裸 JSON 无 envelope;fire-and-forget,不校验 thread 存在):

  ```
  POST /v1/sessions/{thread_id}/feedback
  { "rating": "up" | "down", "comment": "≤4000 字,选填", "turn_seq": 0 }
  ```

  对外集成文档体系 repo 尚无(独立缺口,另立项);业务系统先照本契约接。
- 角色:👍 候选权重最高(人背书);👎+comment 是 SE-A39 的失败语料;打标整体用于
  **校准 SE-A45 judge**(judge 评分与人工标注的一致率是 judge 可信度指标)。

**SE-A44 证据对齐**:implicit / judge-scored 来源候选蒸馏产物即便重放 T1 pass,
**auto-promote 强制降级人审**(`decide_promotion` 加 origin_signal 维度)。理由:输入是
弱标注,先经人审累积信任,数据够了再放开。👍 来源维持可 auto(人显式背书 + 重放独立验证)。

### 12.4 SE-A39 — 👎 变燃料(P0)

`negative_feedback` 不再被进化 worker 无视,但**不单独成蒸馏候选**(仅有失败样本会蒸出「别做 X」
的空洞技能):同 agent 存在 pending 的 positive/implicit 候选时,👎 轨迹(连同 `feedback.comment`,
字段已存在)注入 distiller 的 **failures 对比侧**。👎+success 结局 =「假成功」,对比信息量最高。
仅有 👎 无正例 → 保持跳过。

### 12.5 SE-A42 — 晋升→生效补链(P0,当前最短板)

核实:distilled 技能 ACTIVE 后**仍需 agent manifest 显式声明才被装配** —— 自动晋升的技能没有
任何消费者,`skill_run_usage` 无 with 流量,rollback monitor 无数据,`last_used_at` 不 bump,
30/90 天后被 curator STALE→ARCHIVED。**白蒸馏**。

两案对比后取 B:

- ~~A. promote 时自动改源 agent manifest~~ —— 偷改用户资产、触发 agent 版本变更,否。
- **B. `AgentSpec.auto_attach_evolved_skills: bool`(默认 false,表单露出)**:build 时若开,
  自动附加「本租户 + `evolution_origin='distilled'` + ACTIVE + 蒸馏源 agent_name=本 agent」的
  技能(lazy 档:summary 进 available-skills,不吃 eager prompt 预算)。显式 opt-in、
  per-agent 粒度、不动 manifest 资产。
  实施注记(PR-5 定):附加走**软失败**语义——重名(manifest 显式声明恒优先)、模型
  不匹配、工具名冲突、文本类组件(eager 天性,违背 lazy 契约)一律跳过并 log,绝不因
  自动附加 brick 一个 manifest 合法的 agent;附加项进 `resolved_versions` →
  `bound_distilled_skills`,rollback 监控/curator 活性因此自动成立;字段在模板三层模型
  FIELD_TIERS 归 CAPABILITY 档;`in_session` 来源(agent 显式自建)不自动附加,保持
  manifest 声明制。

附带成立:with 流量 → rollback 监控有数据;last_used_at 有 bump → curator 衰退语义恢复。

**业界对标注记**:技能消费主流是检索式(Voyager embedding top-5 / SkillFlow / selection-based
优于 full-injection 的实证)。lazy 档 = summary 列表 + 模型自选,即 selection-based 的一种,
在**单 agent 蒸馏产物量级(个位~十位)下等价且更简单**。演进触发条件写死:单 agent 附加
技能数 > 20 时,summary 列表本身成为上下文负担,转 embedding/BM25 检索预选(复用 HX-12
工具披露 BM25 先例)。不提前建设。

### 12.5b SE-A47 — 蒸馏入库查重(P2,业界对标补项)

对标发现的 gap:SkillNet / SkillOS / SkillBrew 等技能库系统标配 dedup / merge /
consolidation;Expert Work 的 SkillCurator 只管**衰退**(STALE/ARCHIVED),无去重——同 agent
反复遇到相似问题会蒸出语义重复技能,信号扩容后产量上升,重复堆积会稀释 available-skills
列表并浪费人审带宽。

修:蒸馏产物**入库前查重** —— 与同 agent 现有 distilled 技能(DRAFT+ACTIVE)做 embedding
相似度(embedder 平台级已有);高于阈值 → 不新建技能,转为该技能的**修订轮输入**(走既有
revise 循环,version+1),人审面呈现「更新既有技能」而非新条目。低于阈值正常新建。
合并(merge 两技能为一)不做——修订轮语义已覆盖主场景,真 merge 等库规模需要时再议。
实施注记(PR-7 定):阈值 0.9(比 consolidator 同义 0.85 更严——误合并会把新能力埋进
无关技能的历史);**命中 ACTIVE 目标先翻 DRAFT 重审**——核实发现 `add_version` 即刻
bump `latest_version` 且 bare-name 解析按 latest 服务,未验证修订绝不能在 ACTIVE 技能上
直接生效;auto-promote 通过再翻回(净效果=无缝升级),人审路径自然出现在 DRAFT 队列,
「呈现为更新而非新条目」由此成立(条目=既有技能,历史全在)。deduper 任何故障(embedding
未配置/超时)降级返 None 正常新建——查重是优化不是闸;命中写
`SKILL_EVOLUTION_DEDUP_REVISION` 审计(相似度+目标)。UI 标签语义取「修订待审」
(draft 且 v>1):对 dedup 并入与同蒸馏内多轮 revise 都成立,不虚标「更新既有 ACTIVE」。

### 12.6 SE-A40 — transient 重试档(P0)

现状:蒸馏 processor 任何异常都标 `evolved_at`(幂等换 durability),aux LLM 限流/超时即永久
烧掉候选。修:异常分类 —— transient(限流/超时/连接)不标 `evolved_at`,候选加 `retry_count`
列(migration),worker 下轮重拾;`retry_count >= 3` 或永久性错误才标记放弃。

### 12.7 SE-A41 / SE-A43 — 灰度 + 计量(P1)

- **灰度**:`tenant_config` 键 `skill_evolution_enabled`(默认 false;读法照 `skill_curator`
  读 `skill_stale_days` 先例)。worker / rollback monitor 处理候选前查;平台 `enable_skill_evolution_worker`
  保留为总闸(两层与)。生产路径:开总闸 → 白名单租户逐个开 → live 试点。
  **admin-ui 维度**:开关必须有管理面——system_admin 租户管理页露出「技能自进化」开关
  (含 judge 抽样率输入);不接受「只能 SQL 改 tenant_config」的运维形态
  ([[feedback_iteration_items_need_frontend_audit]])。
- **计量**:distiller / judge / replay 的 aux 调用 token 写 `token_usage`(真 tenant_id、
  agent_name=蒸馏源 agent、evolution 专属 trace_id),替换 `_NULL_TENANT` 占位。进化成本
  进 chargeback 视图,租户账不糊涂。
  **区分维度(实施时定,倾向 A)**:进化用量混进源 agent 的对话成本会让运营分不清 ——
  A. `token_usage` 加 `usage_kind` 列(migration 小、查询显式);B. agent_name 命名空间
  (`skill-evolution:<agent>`,免 migration 但破 per-agent 聚合)。
  **admin-ui 维度**:用量视图按 kind 分列/过滤,进化成本单独可见。
  实施注记(PR-6 定,取 A 案):核实发现 aux 调用**此前完全无计量**(adapter 只把 token
  数装进回复对象,无人落库)——不止是 `_NULL_TENANT` 归因错,是账本上根本没有这笔钱。
  修:worker per-candidate metering scope(contextvar)携带真 tenant/agent/确定性
  trace(`skill-evo-{candidate_id}`),aux 座在回复处记账(never-fail 契约,计量绝不打断
  蒸馏);**replay 主图 build 也穿 `usage_kind='skill_evolution'`**(with/without 重放
  跑的是完整 agent 图,是进化的最大开销,不穿 kind 则本条 admin-ui 要求名存实亡);
  进化行带真 tenant_id 自然进 Y4 月度计价(租户账不糊涂);memory consolidator 的 aux
  同样无计量,属同一 gap 的另一半,另立 backlog 不混进本 wave。

### 12.8 显式不做(本 wave,均有业界对标依据)

- rollback 跨时段对照的统计弱点:业界正解 = shadow / champion-challenger(留部分流量跑
  without 对照),但流量成本对小租户不划算;熔断兜底 + 远期 shadow 是与业界一致的折中。
- 环境可验证信号(工具执行成功率 / 审批通过 / 产物被下载):agent 进化论文群的首选信号,
  但产物下载无审计埋点、工具成功率需归因建模 —— 进 backlog,埋点先行。
- 技能 merge(两技能合一):SE-A47 修订轮语义已覆盖主场景,库规模需要时再议。
- promote-requests 带外触达:事件发射复用 HX-9 webhook(`skill_promote.requested`),
  渠道接入与「审批 pending 通知」backlog 合并立项,不在本 wave。
- 候选队列可观测面(「哪些轨迹在排队蒸馏」的 admin-ui 列表):技能层 review 队列(SE-8)
  已覆盖决策点;候选层先靠 metrics + 日志,试点若证明运营需要下钻再立项。

### 12.9 实施拆分

| PR | 内容 | admin-ui 维度 | 依赖 |
|---|---|---|---|
| D | 本设计 | — | — |
| 1 | SE-A46 打标入口——试点信号主力,先行 | Playground 👍/👎 + comment 框(前端为主) | D |
| 2 | SE-A40 重试档 + SE-A41 灰度(解锁 live 试点) | system_admin 租户管理页「技能自进化」开关 | D |
| 3 | SE-A38/A39/A44 信号扩容(classify 升级 + EVOLVE_SIGNALS + failures 注入 + 弱信号禁 auto) | 无(候选可观测面显式不做,§12.8) | 2 |
| 4 | SE-A45 judge 抽样评分(候选提纯) | 抽样率输入并入 PR-2 开关面 | 3 |
| 5 | SE-A42 auto_attach_evolved_skills(spec 字段 + build 装配) | Agent 表单开关 + i18n | 2 |
| 6 | SE-A43 aux 计量(usage_kind 区分,倾向 A 案) | 用量视图 kind 分列/过滤 | 2 |
| 7 | SE-A47 入库查重(embedding 相似 → 转修订轮) | 人审队列区分「新技能 vs 更新既有技能」 | 3 |
| 8 | promote 事件发射(触达渠道另立项) | 无 | 5 |

验证:各 PR 单测 + 真 PG 集成;收官标准 = **live 试点全场景通过**(§ 12.10),
CI 绿≠live 能跑(★5 纪律)。

### 12.10 live 试点验证计划(收官 checklist)

> 教训在案:9.4 HA live 暴露 5 坑、4.4 真模型 E2E 逮 5 坑 —— live 计划必须先行成文,
> 不是收官时即兴。预期本轮同样逮 3-5 个 live 坑,修完才算 ★5。

**前置**:dev compose 栈 + 真 aux 模型 key(CI 无 model key,真 LLM 场景只能 live 跑);
PR 1-8 全合;灰度:平台总闸开 + `tenant_config.skill_evolution_enabled` 只开 test 租户;
test-agent 开 `auto_attach_evolved_skills`。

**阶段 A — 信号 → 候选**
1. Playground 对 test-agent:打 👍 一条、👎+comment 一条、一条成功对话不打标(等 5 分钟
   不重发)→ 断言 `curation_candidate` 出现 positive / negative / **implicit_success** 三类;
   另在同 thread 5 分钟内重发改口一次 → 断言该轨迹**不**产生 implicit 候选。
2. 造失败 run(max_steps)→ `failed_outcome` 候选。
3. **transient 重试**:临时改坏 aux key → 蒸馏失败 → 断言 `retry_count=1`、`evolved_at` 仍
   NULL;修回 key → 下轮自动重拾成功。连挫 3 次 → 标 evolved_at 放弃。
4. judge 抽样:implicit 候选池过 SE-A45 评分,低分被滤(日志可见),高分保留。

**阶段 B — 蒸馏 → 验证**
5. 👍 候选蒸馏 → DRAFT skill 落库,`skill_eval_result` 有 with/without delta;
   👎 comment 语料出现在 distiller 输入(trace 佐证)。
6. implicit 来源候选:重放即便 pass,**不 auto**,进人审队列(SE-A44)。
7. **查重**:再造语义相似的 👍 候选 → 断言不新建技能,既有技能 version+1(修订轮),
   人审面显示「更新既有技能」。

**阶段 C — 晋升 → 生效**
8. 人审批准 → ACTIVE;test-agent 新会话的 available-skills 出现该技能;`skill_run_usage`
   出现 with 行;`last_used_at` bump —— **生效断链修复的直接证据**。
9. auto 通道:eval_dataset 造 1 条带硬断言的持出 → T1 auto promote + `SKILL_EVOLUTION_AUTO_PROMOTED`
   审计行。
10. 限速:1 小时内制造 >5 次晋升 → 第 6 次降人审;kill-switch engage → auto 全停,
    disengage → 恢复。

**阶段 D — 监控 → 回滚 → 记账**
11. 人为造衰退(改坏已 ACTIVE 技能的 prompt_fragment 后跑真实流量)→ rollback monitor
    判定 → ARCHIVED + 熔断 record + `SKILL_EVOLUTION_ROLLED_BACK` 审计;熔断打开后
    新候选 auto 降人审(共享熔断验证)。
12. **计量对账**:全程 aux token 在 `token_usage` 可查且 tenant/agent 归属正确
    (无 `_NULL_TENANT` 残留),chargeback 视图可见进化成本。

**阶段 E — 运营面**
13. admin-ui:审批队列展示重放证据、kill-switch 交互、Playground 打标控件(axe 无 critical);
    未灰度租户确认全链静默(负路径:候选不生成、worker 不处理)。
14. webhook endpoint 收到 `skill_promote.requested` 事件。
