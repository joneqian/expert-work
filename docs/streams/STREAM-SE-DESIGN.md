...
- **回归回滚(SE-A11,拆 SE-7d-1/2/3)**:promote 后按 **专表 `skill_run_usage`(§4.4)** 归因该 skill_version 关联 run 的 outcome,滚动窗口内成功率显著下降 → 自动 archive(对标报告 § 7 防坍缩)。**按 version 判定不连坐**。
  ...
  - **SE-7d-3 monitor + 动作**:拆 SE-7d-3a(gate,CI 可测)+ SE-7d-3b(monitor 循环 + 埋点 + 接线,真路径)。
    ...
    - **SE-7d-3b-ii 埋点**(侵入 orchestrator run 热路径,真路径 integration):`BuiltAgent.bound_distilled_versions` ← build 时 `_LoadedSkills.resolved_versions` distilled 子集 → 经 RunRecord → `sse.py` run 收尾 finally(已有 outcome + thread_id)经 `SkillRunUsageRecorder` seam 落 `skill_run_usage` + control-plane 实现 + 接线。把数据喂进 monitor。
