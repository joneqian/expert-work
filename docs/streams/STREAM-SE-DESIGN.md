...
- **回归回滚(SE-A11,拆 SE-7d-1/2/3)**:promote 后按 **专表 `skill_run_usage`(§4.4)** 归因该 skill_version 关联 run 的 outcome,滚动窗口内成功率显著下降 → 自动 archive(对标报告 § 7 防坍缩)。**按 version 判定不连坐**。
  ...
  - **SE-7d-3 monitor + 动作**:拆 SE-7d-3a(gate,CI 可测)+ SE-7d-3b(monitor 循环 + 埋点 + 接线,真路径)。
    ...
    - **SE-7d-3b monitor 循环 + 埋点 + 接线**(真路径,integration):monitor 跨租户(owner RLS 豁免,同 curator)枚举 ACTIVE distilled version + 取 promote 基线(最近 pass `skill_eval_result.skill_score`)→ 逐个过 gate;埋点在 orchestrator `sse.py` run 收尾 finally(已有 outcome + thread_id)按 build 时 `_LoadedSkills.resolved_versions` 的 distilled 子集落 `skill_run_usage`;app lifespan 装配 monitor。
