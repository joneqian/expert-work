...
> Admin UI 是 expert-work 给操作人群(平台 admin / agent 开发者 / 运营)的产品级前端入口。
> Business 系统通过 API 消费 Expert Work 的 per-user 持久 agent 能力 —— **Expert Work 自身不给末端用户做 UI**;
> 末端用户通过 business 系统自己的 UI 与 agent 对话(见 [memory:target-product-form](../../.claude/projects/-Users-mac-src-github-jone-qian-expert-work/memory/project_target_product_form.md))。
...
消费 agent,Expert Work 自身不需要给末端用户做对话/工作台 UI。原 H.4 的 memory CRUD / artifact 浏览
/ 沙盒会话查看 等 backend 能力**保留**(K6 memory / J.9 artifact / J.15 sandbox API 不动),
但**改由 business 系统的 UI 消费**;Expert Work Admin UI 中作为 admin 视角的"跨 user / 跨 agent
...
| **H.1b React 19 + Vite + Antd 5 骨架**(5-7 天) | 仓库根 `apps/admin-ui/`(Vite 工程);Antd 5 ConfigProvider 把 `--ew-*` token 接入 design token;i18n(zh-CN/en);路由(`react-router-dom` v7);鉴权(API Key / JWT);CommandPalette(`kbar`);Lucide 图标;dark/light theme toggle | 接 control-plane `/v1/*` API |
...
│   │   └── antdTheme.ts      (Antd ConfigProvider 主题,把 --ew-* 映射到 Antd token)
│   ├── i18n/
...
**Decision**:用 Antd 5 + Expert Work 设计基线 override —— Antd 自带 a11y / i18n / form / table 全套,我们只要 token + override 几个关键组件细节即可。
...
**Context**:debug 能力是 Expert Work 操作人群的核心诉求;放哪里取决于谁需要看 + 看的时候手边有什么。
...
**Decision**:用 CSS custom properties(`--ew-*`)+ Antd ConfigProvider 注入 token + 极少 inline override。**不引入 Tailwind**(与 Antd 默认样式冲突 + 学习成本);**不用 styled-components**(运行期成本 + 与 Antd 主题割裂)。
...
- `RunStore` ABC 加两个抽象方法,InMemory + SQL 各加实现:
  ...
  - `list_all_tenants(*, status, limit, offset) -> list[RunInfo]` — 跨租户;调用方 MUST 包 `bypass_rls_session()`(Stream N 同 [trigger sql.py:117](../../packages/expert-work-persistence/src/expert_work/persistence/trigger/sql.py))
...
- **producer 接入**:`run_agent` 工作循环 publish 到 bridge 同步**双写**到 `run_event`(在 `bridge.publish` wrapper 里加 hook,or `run_agent` 直接调用 store)。**失败策略**:store 写失败 → log warning + skip(SSE 不阻塞,事件流是首选);store 写失败计数器 `expert_work_run_event_persist_errors_total`。
...
| 文件 | 变更 | 行数估 |
|---|---|---|
| `packages/expert-work-runtime/src/expert_work/runtime/runs/store.py` | ABC + InMemory + SQL 加 `list_for_tenant` / `list_all_tenants` | +85 |
| `packages/expert-work-runtime/tests/test_run_store.py` | +`test_list_for_tenant_filter_by_status` / `_pagination` / `_tenant_isolation` / `test_list_all_tenants_returns_all` / `_with_status_filter` | +60 |
...
| 文件 | 变更 | 行数估 |
|---|---|---|
| `packages/expert-work-persistence/migrations/versions/0037_agent_run_trace_id.py` | **新文件** — `ALTER TABLE agent_run ADD COLUMN trace_id varchar(32) NULL` + partial index `(trace_id) WHERE trace_id IS NOT NULL` | +35 |
| `packages/expert-work-persistence/src/expert_work/persistence/models/agent_run.py` | 加 `trace_id: Mapped[str | None] = mapped_column(String(32), nullable=True, index=False)` | +2 |
| `packages/expert-work-runtime/src/expert_work/runtime/runs/schemas.py` | `RunInfo` dataclass 加 `trace_id: str \| None = None` | +2 |
| `packages/expert-work-runtime/src/expert_work/runtime/runs/store.py` | `RunStore.set_trace_id(*, run_id, tenant_id, trace_id)` 抽象 + 2 实现;`_row_to_dto` / `create` 透传 | +50 |
| `packages/expert-work-runtime/src/expert_work/runtime/runs/manager.py` | `RunManager.create` 之后立刻调 `set_trace_id(record.run_id, tenant_id, current_trace_id_hex())` | +6 |
| `packages/expert-work-runtime/tests/test_run_store.py` | +`test_set_trace_id_writes_and_reads_back` / `_idempotent_overwrite` / `_tenant_isolation` | +50 |
...
| 文件 | 变更 | 行数估 |
|---|---|---|
| `packages/expert-work-persistence/migrations/versions/0038_run_event.py` | **新文件** — 新建 `run_event` 表(`run_id uuid` + `seq bigint` + `event_name text` + `data jsonb` + `created_at_ms bigint` + `created_at timestamptz`)+ `PRIMARY KEY (run_id, seq)` | +55 |
| `packages/expert-work-persistence/src/expert_work/persistence/models/run_event.py` | **新文件** — SQLAlchemy `RunEventRow` model | +35 |
| `packages/expert-work-runtime/src/expert_work/runtime/runs/event_store.py` | **新文件** — `RunEventStore` ABC + `InMemoryRunEventStore` + `SqlRunEventStore`;方法:`append(run_id, tenant_id, event_name, data, seq, created_at)` / `list(run_id, tenant_id, since_seq=0, limit=1000)` | +180 |
| `packages/expert-work-runtime/tests/test_run_event_store.py` | **新文件** — 8 测(append + list / pagination / tenant_isolation / since_seq / cross-tenant)| +150 |
| `services/orchestrator/src/orchestrator/sse.py` | `run_agent` worker 在 `bridge.publish` 处双写 `RunEventStore.append`;失败 → log warning + `expert_work_run_event_persist_errors_total.inc()`;不阻塞 | +30 |
...
| Prometheus counter `expert_work_run_event_persist_errors_total` 已声明,只加 `.inc()` 调用 | — | +0 |
...
| `run_event` 持久化失败(DB 不可达) | bridge.publish 继续,event_store.append 抛 | log warning + `expert_work_run_event_persist_errors_total.inc()`;事件流可能漏帧但不阻 SSE |
...
| Prometheus counter `expert_work_control_plane_run_list_total{tenant_scope}` | 每次调用 | `tenant_scope ∈ {home, cross, target}` |
| Prometheus histogram `expert_work_control_plane_run_list_seconds` | 每次调用 | (无标签 — 与现有列表端点 latency 系列对齐)|
| Prometheus counter `expert_work_run_event_persist_errors_total{backend}` | PR 3:`RunEventStore.append` 抛错 | `backend ∈ {memory, sql}` |
| Prometheus counter `expert_work_run_event_persist_total{backend, event_name}` | PR 3:`RunEventStore.append` 成功 | 关注热点 event_name 频次 |
| Prometheus histogram `expert_work_control_plane_run_events_endpoint_seconds{mode}` | PR 4:`GET .../events` 调用 | `mode ∈ {live, replay}` |
...
- **缺**:**RunsList 页 mockup**(`mockups/09-runs-list.html`)和 **ApprovalCard with override_args 编辑态 mockup**(可叠在 `04-run-trace.html` 同一文档加 § / 加新文件)。
  - **决策**:**先实现,不另出 mockup**。理由:本 PR 已用 ASCII layout(§ 6.5.3)+ 状态机(§ 6.5.9)锁定结构;`tokens.css` + Antd 5 + Expert Work override 三层映射在 H.1a 已锁,RunsList 与 AgentsList 视觉同型(都是 cross-tenant aware 表格 + 顶 toolbar),无新视觉元素。
...
- **E** EventStreamPanel 默认折叠 + localStorage `expert_work.runDetail.eventStream.expanded` 持久化 ✅
...
4. **可观测齐全** — 新 metrics: `expert_work_runs_list_total` / `expert_work_run_event_persist_total` / `expert_work_run_event_persist_errors_total`;audit:`RUN_LIST_READ`(per § 6.5.11)
...
- **`AuditAction` enum 加 `AUDIT_QUERY = "audit:query"`**(单源 `packages/expert-work-protocol/src/expert_work/protocol/audit.py`)
...
4. 可观测齐全:PR3 新增 `expert_work_control_plane_audit_query_total` + `_seconds`;`AUDIT_QUERY` audit
5. CI 全绿:11/11 检查
...
- `packages/expert-work-protocol/src/expert_work/protocol/audit.py` `AuditAction.AUDIT_QUERY` + ResourceType Literal 加 `"audit"`
...
**新 Prometheus metrics**(PR3,通过 `expert_work.common.observability.expert_work_counter` / `expert_work_histogram` helper,**不直接 import `prometheus_client`**):
- `expert_work_control_plane_audit_query_total{tenant_scope, result}` — counter
- `expert_work_control_plane_audit_query_seconds{tenant_scope}` — histogram
...
4. **可观测齐全** — PR 3 加 2 Prometheus(`expert_work_control_plane_audit_query_total` + `_seconds`);所有 mutation 经既有 audit_emit(MEMORY_UPDATE / CURATION_PROMOTE / SKILL_CREATE / TRIGGER_CREATE / SERVICE_ACCOUNT_CREATE / ROLE_BINDING_CREATE / QUOTA_CONFIG_WRITE / TENANT_CONFIG_WRITE)
