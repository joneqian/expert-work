...
| 子项 | 实现内容 | 关联子系统 / P0 |
|------|---------|-----------------|
| **E.1 LangGraph PostgresSaver 接入** | 在 `services/orchestrator/` 新服务中用现成 `expert-work-runtime/checkpointer.make_checkpointer`（A.2 已建）构建 `GraphRunner`；首次 LangGraph `StateGraph.compile(checkpointer=...)` 跑通；重启可 resume；checkpoint TTL 由 Stream D.3 retention 兜底。 | 01-system § Orchestrator；GAPS § n/a |
| **E.2 `@Next/@Prev` 锚点系统** | 新 `packages/expert-work-runtime/middleware/`：`Middleware` Protocol + `MiddlewareChain` + `anchor` decorator。中间件声明 `name`、可选 `after: list[str]` / `before: list[str]`；链构建期做拓扑排序；环 → `ChainCycleError`。**所有后续中间件（E.3-E.5 / E.10 / D.2 PII / E.13 cache）都注册到链上**。 | research/05 §"@Next/@Prev"；01-system §"中间件链" |
...
| **E.13 LLM response cache** | 精确匹配 cache：key = `sha256(tenant_id || model || normalize(messages) || temperature || max_tokens)`；Redis backend（基础设施已建）；`cache_ttl` 默认 3600s，per-agent 可覆盖；**per-tenant 命名空间**（无 cross-tenant 命中风险）；非 deterministic 调用（temperature > 0.1）默认绕过；命中 `cache_hit` audit + metric `expert_work_llm_cache_hit_total{tenant,model,result}`。 | GAPS § 10 #28 |
...
1. **1 个 minimal agent 跑通 builtin / HTTP / MCP 三类工具** — `manifest.yaml` 声明 `tools: [web_search, http, mcp:fs]`，发 prompt `"搜一下 expert-work 的 GitHub stars 数，然后 GET status.github.com 看是否正常，最后 ls 当前目录"` → SSE 流回完整 ReAct trace。
...
3. **PII redactor 工作** — `details: {ssn: "123-45-6789"}` 入 prompt → Langfuse + audit log 显示 `***REDACTED***`；命中 `expert_work_audit_redact_hit_total{pattern="pii_field"}`。
...
│   └──────────────────────────────────────────────┘ │
│   RunManager / StreamBridge（A.2 expert-work-runtime 库） │
└────────┬───────────────────────────┬───────────────┘
         │ write audit                │
         ▼                            ▼
┌──────────────────┐   ┌──────────────────────────────────────┐
...
```python
# packages/expert-work-runtime/src/expert_work/runtime/middleware/base.py
...
```python
# packages/expert-work-runtime/src/expert_work/runtime/cancellation.py
...
```python
# packages/expert-work-runtime/src/expert_work/runtime/llm/cache.py
...
- **选择**：每个工具的适配器内部 truncate；通过 `expert_work.runtime.tools.truncation` 共享 helper（≈30 LOC）复用 head-only / tail-only / middle-bias 三种策略。
...
```python
# 实际：expert_work.runtime.stream_bridge.base.StreamEvent
@dataclass(frozen=True)
class StreamEvent:
...
```yaml
# packages/expert-work-protocol/src/expert_work/protocol/manifest.py
llm:
  primary:
    provider: anthropic
...
| LLM 断路器开错 → 整 tenant 不能用 | 业务中断 | per provider key 粒度；fallback 链兜底；告警 P0 `expert_work_llm_circuit_open_total > 0`|
...
E.2  feat(e-2): MiddlewareChain + anchor 系统
     - packages/expert-work-runtime/src/expert_work/runtime/middleware/{base,chain}.py
     - 拓扑排序 + 环检测
...
E.10.5 feat(e-10-5): loop_detection_middleware
       - packages/expert-work-runtime/.../middleware/loop_detection.py
