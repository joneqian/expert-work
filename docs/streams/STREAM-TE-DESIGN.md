...
> 把"改进版 Tool 执行引擎"设计落地到 Expert Work:在保留 Expert Work 已成熟的并行调度的前提下,
...
源自一篇 Go「Harness(驾驭工程)」专栏(Tool Registry / 极简原语 / Parallel Tool Calling / 文件并发安全)。经 5 图事实分析 + 三个真实实现(`openclaw` TS / `deer-flow` Py / `hermes` Py)源码对照 + Expert Work 三路现状探查,形成本设计。**结论:文章正确内核保留,其"本地可信 / 极简 4 原语唯一解 / 押注模型纪律"三个前提在多租户 server-side 下解开。** Expert Work 定位 = 受治理的多租户 agent 平台(非本地全能 CLI),企业办公约占 70%(读多写少)。
...
- **并行调度** ✅ **已成熟**:`tools/scheduling.py`(Stream L.L6)`plan_stages` + `is_read_only`/`path_args` 冲突检测 + 路径归一化(`PurePosixPath.as_posix`)+ `builder.py:395` `asyncio.gather` + Semaphore(8);子 agent 并行(J.4 `is_parallel_safe`)。缺:`irreversible` 串行档。**——这是文章最难的部分,Expert Work 已有,不重做。**
...
- **沙箱** ✅ 强:`sandbox-supervisor`(gVisor+tmpfs+per-user workspace `expert-work-ws-{tenant}-{user}` `persistence/workspace/base.py`+超时 30/300s+quota)。**三家参考实现都没有这个级别的多租户隔离。**
...
- **可观测**:per-tool Prometheus(`expert_work_tool_call_total{tool,outcome}` / `expert_work_tool_latency_seconds{tool}` / `expert_work_tool_error_total{tool}`,label 控基数不含 tenant 高基维)+ 每工具 Langfuse span + trajectory 富化(exit_code / 读写路径集 / 耗时)。
...
- **分工**:匹配降级管"模型自己对不上"(可用性),CAS 管"别人并发改了"(正确性),两者都要。三家参考最强也只 advisory stale(hermes),硬 CAS 是 Expert Work 差异化。
...
warm session 命中(per-tool `expert_work_tool_latency_seconds{tool}`,TE-3 已采集,文件原语作为 tool 自动覆盖):
...
**完成 = Expert Work TOOLS 引擎产品级**(治理/隔离/审计/并发正确性齐备,且办公工具面补足)。
