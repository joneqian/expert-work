...
| 文件 | 职责 | 本次改动 |
|---|---|---|
| `services/orchestrator/src/orchestrator/graph_builder/streaming_redact.py` | 流式脱敏(buffered-release) | Task 2:新增 `RESCAN_LOOKBACK`/`WINDOW` 常量、重写 `StreamingRedactor` 状态与 `feed`/`flush`、加 `_advance_frozen`(screen/DLP 同扫未冻结尾) |
...
**Files:**
- Modify:`services/orchestrator/src/orchestrator/graph_builder/streaming_redact.py:29-89`(常量块 `HOLD_CHARS` 之后新增两常量;重写 `StreamingRedactor` 的 `__init__`/`_redact`/`feed`/`flush`,新增 `_advance_frozen`)
...
- Produces:`StreamingRedactor` 公开签名不变(`__init__(*, dlp, screen)` / `feed(text) -> str` / `flush() -> str`);内部新私有方法 `_advance_frozen(self, tail_red: str) -> None`(screen 与 DLP 同扫未冻结尾 `_buf[_frozen_raw:]`);新状态 `_emitted_out`/`_frozen_raw`/`_frozen_out`(替换旧 `_emitted_len`)。
...
def test_screen_latch_survives_large_single_delta() -> None:
    # 命门 3 回归:单个大 delta(credential + ≥HOLD 尾字符)会把 credential 头推到
    # 固定窗外→漏扫→emission 越过发出→泄漏。screen 改扫未冻结尾 _buf[_frozen_raw:]
    # 后捕获(_frozen_raw 恒 ≥HOLD 落后 end,credential 完成帧整条在尾内)。
    r = StreamingRedactor(dlp=False, screen=True)
    key = "AIza" + "B" * 35  # Google API key 形状,命中 _SECRET_PATTERNS
    out = r.feed("x" * 200)                        # 安全前缀,emission 追平
    out += r.feed(" " + key + " " + "z" * 90)      # 一个大 delta:key + 长尾
    out += r.flush()
    assert key not in out                          # 凭据从不 emit
```
...
#: Frozen-pointer target: everything before a *verified-clean* frozen boundary at
#: ``end - WINDOW`` is finalized and never rescanned again — this is what makes
...
> 注意:`screen` 检查现扫**未冻结尾 `_buf[_frozen_raw:]`**(非固定窗)——等价性由"`_frozen_raw` 恒 ≥HOLD 落后于 end,凭据 min-match 完成那帧整条必在尾内被抓、先于其头 emit"保证(HOLD(64)>39,见 spec §2)。**为何不用固定窗**:单个大 delta(credential + ≥HOLD 尾)会把 credential 头推到固定窗外→漏扫→泄漏;尾扫对任意大 delta 成立。`_window_start` 辅助方法遂删除。旧 `feed`/`flush` 里的 `self._buf += text`、`if not text` 提前返回、`_blocked` 提前返回、`max(...)` clamp 语义全部保留。

- [ ] **Step 4: 运行整测文件,确认全绿**

Run: `cd services/orchestrator && uv run python -m pytest tests/test_streaming_redact.py -v`
Expected: PASS —— 全部现有测 + Task 1 四测 + 本 Task 四新测(O(n) 有界、折叠守卫、冻结洁净假阳性)全绿。
...
**3. Type consistency:** `_emitted_out`/`_frozen_raw`/`_frozen_out`(int)、`_advance_frozen(tail_red: str)->None`(feed 以 `self._advance_frozen(tail_red)` 传入已算好的 `tail_red`)、`feed(str)->str`、`flush()->str`、模块级 `WINDOW`/`RESCAN_LOOKBACK`(int)在 Task 2 内一致;测试引用 `streaming_redact.WINDOW`/`sr.scan_and_redact`/`sr.screen_output` 与生产模块符号一致。旧 `_emitted_len` 被 `_emitted_out` 完全取代(无残留引用——旧测不引用私有状态,仅黑盒)。✅
