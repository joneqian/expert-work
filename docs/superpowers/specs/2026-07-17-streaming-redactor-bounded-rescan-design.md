...
RESCAN_LOOKBACK = 64       # 新增:冻结指针目标滞后量(= 每帧 DLP 重扫尾长),性能界
WINDOW = HOLD_CHARS + RESCAN_LOOKBACK   # = 128:冻结指针目标 `end - WINDOW`
```

> 注:WINDOW 只影响**性能**(冻结指针目标 / 每 feed 重扫量),**不影响任何正确性**:① bounded 等价由 `_advance_frozen` 的精确分割等价判据保证 ② screen latch 由扫**未冻结尾**保证(靠 `HOLD(64)>39`,与 WINDOW 无关,见 §2)。故 `RESCAN_LOOKBACK` 无正确性下界,取 64 只为把每帧重扫尾控制在 ~2·WINDOW。
...
def feed(self, text: str) -> str:
    ...
    if self._screen and screen_output(tail).blocked:  # 扫未冻结尾,非固定窗(见更正)
        self._blocked = True
        return ""
...
- screen 与 DLP 均作用于同一未冻结尾 `tail = _buf[_frozen_raw:]`(先 screen 后 redact),不设固定窗。
...
flush 释放 hold 内残余(不再扣 HOLD_CHARS),语义同旧 flush。（`_window_start()` 辅助方法在 screen 改扫未冻结尾后已无用,删除;`_advance_frozen` 内联 `max(0, len(_buf) - WINDOW)`。）
...
2. **screen latch 不漏(命门 3 —— 曾错、已更正)**:screen 必须扫**未冻结尾 `_buf[_frozen_raw:]`**,**不是**固定窗 `_buf[end-WINDOW:]`。固定窗的漏洞:单个大 delta(credential + ≥HOLD 尾字符)会把 credential 头推到窗外 → 漏扫;而 emission 于同帧越过 credential 发出 → 与全扫分歧、泄漏(实证:`feed("x"*200)` 后 `feed(" AIza…{35} " + "z"*90)` 泄漏 key)。**尾扫修复**:`_frozen_raw` 恒 ≥ HOLD 落后于 end,故 credential 的 min-match 完成那帧,`_frozen_raw ≤ end-HOLD < credential_start`(`credential_start ≥ end-39`,`HOLD(64)>39`)→ **整条 credential 必在尾内** → 当帧被抓、latch,先于其头 emit。对任意大 delta 成立,且无额外渐近开销(该尾本就每帧 redact)。经 30000 次随机分片(含 200 字符大 delta)对照 `screen_output(whole)` 验证不泄漏。
...
7. **screen 大 delta latch**:安全前缀释放到位后,单帧 `feed(" AIza…{35} " + "z"*90)`(credential 后跟 ≥HOLD 尾)→ `key not in out`。固定窗实现会泄漏,尾扫修复捕获(命门 3 回归)。
