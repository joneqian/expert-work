...
要点(经代码核实):
- **改在②fork**,整租户共享;**③实例不存 spec**,运行期按 user_id 拉 fork build。per-user 隔离今天已按 `(tenant_id, user_id=tenant_user.id)` 键记忆(`MemoryStore`)/工作区卷(`expert-work-ws-{tenant}-{user}`)/线程,实例层零 spec 复制。
