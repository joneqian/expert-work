...
未选(backlog):配置临时覆盖再跑 / 模拟触发器 payload / 全工作区文件树浏览。

**#5 审批闸内联(PR5,已交付)**:agent 命中 `approval_required_tools`/`ask_for_approval`
→ run 暂停(`RunStatus.PAUSED`,写 `agent_approval` 行),SSE `end` 帧不带数据。故调试台
在**每轮流结束后查 `listApprovals(status=pending)` 过滤本 thread**(带小重试覆盖
注册竞态)→ 命中则在该轮渲轻量审批卡(action_summary/node/proposed_args 只读 +
批准/拒绝;改参仍在 RunDetail 的 Monaco 卡)。决策走 `decideApprovals`(返
`continuation_run_id`)→ `streamRunEvents(thread, continuationRunId)` 把续跑事件**追加到同一轮**
→ 续跑结束再查一次(链式多闸)。复用既有审批后端 + SDK,零后端改动。

**iter2(已交付,PR4)**:补 #4 余(每轮**延迟**=帧墙钟 / **step_count**=节点最大 /
**成本** ¥=按模型 rate card 算,#774 已砍 plan_tier 故 per-(provider,model)直查)+
**#8** 每轮跳 `/runs/{threadId}/{runId}`(run_id 取自 metadata 帧)+ **#6** 加载历史会话
续聊(`listSessions` 取本 agent 线程下拉 → resume,过去轮次不回放,banner 提示)。
