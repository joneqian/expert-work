...
> **主租户(合成平台租户)不进 switcher**(Stream ACCT)。`/setup` 建的平台管理员户口落在合成平台租户(`1111…`),它无工作区、纯户口;平台层级即 `"*"` scope,不该再作为 peer 行列出(同 Slack Enterprise Grid / Grafana Server Admin 的"上一层而非旁边一个"呈现)。`/v1/me` 返 `home_is_platform`,前端据此:① switcher 隐藏 home 选项;② 把这类管理员的默认 scope 从 `home` 提到 `"*"`(否则 Select 指向一个不存在的选项)。**双重身份**管理员(真公司租户成员被 GRANT 平台权限)`home_is_platform=false`,home 选项保留。
