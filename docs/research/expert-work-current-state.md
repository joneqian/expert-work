# expert-work 当前实现状况报告（15 维度同口径）

> **本报告作用**：把 Expert Work 当前实现按和 `hermes-deep-dive.md` 完全相同的 15 维度逐项盘点，作为后续 "Expert Work vs Hermes gap analysis" 的**Expert Work 这一边的事实底稿**。
>
> **范围声明**：
> - 仅描述 Expert Work 当前实现的事实，**不**与 Hermes 做对比，**不**给"借鉴 / 应该补"清单 —— 那是下一份报告的工作。
> - 只评判事实，**不**评判 Expert Work 设计的好坏。
...
>
> **源码版本**：`expert-work` @ `6e0e9edaa60b4fe91e3a00140ffeda64d766aec9`（HEAD on 2026-05-27）
> **报告生成日期**：2026-05-27
> **配套报告**：`docs/research/hermes-deep-dive.md`（同维度的 Hermes 事实底稿）

## 阅读指引

- **从第 0 章入**：因为 Expert Work 是分布式微服务架构（与 Hermes 单进程 CLI 完全不同的形态），第 0 章会把整个 monorepo 的服务拓扑、依赖关系、当前所处的里程碑阶段讲清楚，后面 15 维度章节就不再重复解释。
- **每章统一 6 节模板**：
  1. **现状评级** — `生产级 / 部分实现 / 已设计未实现 / 不在 scope`（一句话定性 + ITERATION-PLAN 的 M0/M1/M2/M3 阶段标签）
  2. **设计立场** — Expert Work 在这个维度的核心理念
...
- **现状评级的含义**：
  ...
  - **不在 scope**：这是 Expert Work 设计阶段做出的有意识取舍，**不是缺失**
...
> "Expert Work — 业务无关的多租户企业 Agent 执行引擎，用于替代 Dify。已有完整架构设计（Brain-Hands-Session 三层解耦）、4 阶段路线图（M0→M3）、24 项产品级 P0 基础设施清单"

这一行说清了 Expert Work 的几个**根本性设计立场**：

1. **业务无关**：不绑定任何具体业务领域（不像 Dify 是对话产品），仅提供 agent 执行引擎；
2. **多租户企业级**：从第一天就考虑 tenant_id 全链路、RLS、quota；
3. **替代 Dify**：意味着 Expert Work 的"客户"是希望自己控制 agent 运行栈、不想用 SaaS 的企业；
...
```
expert-work/                                        # 6e0e9ed @ 2026-05-27
├── packages/                                       # 4 个共享 Python SDK
│   ├── expert-work-common/                               # observability / metrics validator / lifecycle
│   ├── expert-work-persistence/                          # SQLAlchemy 2.0 + Alembic（migrations 0001..0038）
│   ├── expert-work-protocol/                             # Pydantic DTO 协议层
│   └── expert-work-runtime/                              # middleware chain / sandbox provider / secret store
│
├── services/                                       # 7 个独立部署的服务
...
数据库 migration 数量：**38 个 Alembic 版本**（`packages/expert-work-persistence/migrations/versions/0001..0038`）。
...
| ORM | SQLAlchemy 2.0 + Alembic | `packages/expert-work-persistence/migrations/` |
...
也就是说：**Expert Work 当前已经把 Hermes 的 8 条单 turn 能力（L1-L8）作为生产级实现进了 main**，本报告会把这些显式标出。

## 0.6 ITERATION-PLAN 的两条硬性"自律"

读后面每个维度的"局限"小节前要知道：Expert Work 团队对自己的开发流程有两条**硬性约束**（`docs/ITERATION-PLAN.md:39-97`）：
...
└──────┬───────────────┘                       └──────────────────────┘
       │
       │ docker run -i + expert-work-sandbox-egress network
       ▼
┌──────────────────────┐
│  Sandbox 容器        │
│  (runsc + readonly + │
...
- **沙箱出站走 `expert-work-sandbox-egress` Docker `--internal` 网络**（F.9）：连 `169.254.169.254` 这类元数据服务被拒；外部 HTTP 必须经 credential-proxy。

## 0.8 数据库表速查

按 `packages/expert-work-persistence/migrations/versions/` 命名顺序（截至迁移 0038）：
...
Expert Work 的自我改进设计跟 Hermes 是**完全不同的哲学**：

- **Hermes**：会话结束 → 后台 fork agent → 自动决定要不要写 memory / 改 skill → daemon thread 写盘。**Agent autonomy 优先**。
- **Expert Work**：会话结束 → 自动落 trajectory → 自动按 signal 生成 candidate → **必须人工审 → 才能 promote 进 eval dataset**。**Audit / governance 优先**。

这跟 Expert Work 的产品定位（"业务无关的多租户企业引擎"）一致 —— 企业客户不接受 agent 自己改自己的 skill 库 + memory 没人审核。
...
| Feedback 持久化 | `packages/expert-work-persistence/src/expert_work/persistence/feedback_store.py` | `FeedbackStore` ABC + SQL impl |
| 轨迹记录器 | `services/orchestrator/src/orchestrator/trajectory/recorder.py` (286 行) | `TrajectoryRecorder` |
| Curation Candidate DTO | `packages/expert-work-protocol/src/expert_work/protocol/eval_dataset.py` | `CurationCandidateRecord` |
| Eval Dataset DTO | 同上 | `EvalDatasetRecord` |
| 迁移 | `packages/expert-work-persistence/migrations/versions/0034_eval_dataset.py` | `eval_dataset` / `curation_candidate` 表 |
...
[G.4 Eval Gate]
  └─ tools/eval/expert_work_eval.py
       └─ 加载 YAML dataset
       └─ 跑 agent 对比 expected
...
`packages/expert-work-protocol/src/expert_work/protocol/eval_dataset.py`（推断结构）：
...
`packages/expert-work-persistence/src/expert_work/persistence/models/skill.py:31-66`（K6 spot-check 确认）：
...
也就是说 Expert Work 已经为"agent 自动写 skill"留好了 schema 接口（status=draft），但 M0 不开放 agent 写入。
...
- **没有 background fork**：Expert Work 不会在会话结束时自动 fork 一个新 agent 反思。
...
跟 Hermes 的"per-`$HERMES_HOME` 全局共享 MEMORY.md"完全不同，Expert Work 的核心约束是：
...
5. **Memory 不在系统提示快照**：跟 Hermes 把 MEMORY.md 注入系统提示不同，Expert Work 是 agent_node 跑前 `memory_recall_node` 显式调 `MemoryStore.recall()` 拿 top-k，作为 `recalled_memories` channel 传给 agent_node，最后渲染进 system context。

## 3.3 关键代码路径

| 角色 | 文件 | 关键符号 |
|------|------|---------|
| MemoryStore Protocol | `packages/expert-work-persistence/src/expert_work/persistence/memory/base.py` | `class MemoryStore(ABC)` |
| SQL 实现 | `packages/expert-work-persistence/src/expert_work/persistence/memory/sql.py` | `SqlMemoryStore` + pgvector |
| InMemory 实现 | `packages/expert-work-persistence/src/expert_work/persistence/memory/memory.py` | 单测用 |
| DLQ Worker | `packages/expert-work-persistence/src/expert_work/persistence/memory/dlq.py` + `control_plane/memory/` | `MemoryDLQWorker`（lifespan task） |
...
| Embedder Protocol | `packages/expert-work-runtime/src/expert_work/runtime/embedder.py`（推断） | `Embedder` Protocol |
| Memory API（CRUD） | `services/control-plane/src/control_plane/api/memory.py` | `GET/PATCH/DELETE /v1/memory` + `/v1/memory/{id}` |
| 迁移 0013 | `packages/expert-work-persistence/migrations/versions/0013_memory.py` | `memory_item` 表 + pgvector 索引 |
...
expert_work_memory_dlq_cycle_errors_total
expert_work_memory_dlq_dead_letters_total
expert_work_memory_dlq_retries_succeeded_total
```
...
- **没有 frozen snapshot**：跟 Hermes 的"加载时冻结 + 系统提示注入 + 中毒条目替换为占位符"不同，Expert Work 的 memory 是 per-turn 动态召回，所以**前缀缓存命中率天然低**（每 turn 召回不同的 top-k）。
...
- **MEMORY.md 风格的"快照型记忆" / "USER.md 风格的角色档案" 不存在**：Expert Work 没有这种"系统提示常驻的高密度文本块"概念，所有 memory 都是动态召回。
...
跟 Hermes 的"最多 3 轮多 pass 压缩"对应 —— Expert Work `max_passes` 默认也是 3（Hermes 经验值的直接移植，因为 Stream L 就是从 Hermes 学的）。
...
| Dynamic context middleware | `packages/expert-work-runtime/src/expert_work/runtime/middleware/dynamic_context.py` | `DynamicContextMiddleware`（E.3） |
| Token usage middleware | `packages/expert-work-runtime/src/expert_work/runtime/middleware/token_usage.py` | `TokenUsageMiddleware`（G.9） |
| Token usage store | `packages/expert-work-persistence/src/expert_work/persistence/token_usage_store.py` | `SqlTokenUsageStore`（migration 0036） |
...
vs Hermes 的 `SUMMARY_PREFIX`（300+ 字符的复杂控制提示），Expert Work 的摘要 prompt **简短得多** —— 只给摘要器，不给主 agent。摘要本身落在 `<context-summary>` SystemMessage 里作为 reference。
...
- **summary 是 SystemMessage 不是 HumanMessage**：跟 Hermes 选择"summary HumanMessage 前置到 tail 首条 user msg"不同 —— Expert Work 用独立 SystemMessage，保护 leading SystemMessage 的字节稳定性（L1 prompt caching 不变式）。
...
manifest 字段（`packages/expert-work-protocol/src/expert_work/protocol/agent_spec.py` 推断）：
...
`packages/expert-work-runtime/src/expert_work/runtime/middleware/token_usage.py` `TokenUsageMiddleware`：

- `after_llm_call` 中间件，从 `AIMessage.usage_metadata` 提取 input/output/cached tokens；
- emit Prometheus counter `expert_work_llm_token_usage_total{tenant, agent, model, type}`；
...
agent_node 入口
  ...
  └─ after_llm_call middleware chain
        ...
        │     ├─ expert_work_llm_token_usage_total{tenant, agent, model, type=input}
        │     ├─ expert_work_llm_token_usage_total{tenant, agent, model, type=output}
        │     ├─ expert_work_llm_token_usage_total{tenant, agent, model, type=cached_input}
...
- **图像 token 估算缺失**：跟 Hermes 把每图算 1600 token 不同，Expert Work 的 `estimate_tokens` 看起来不专门处理图像 message（J.6 多模态 message 进 messages 后估算可能偏低）。
...
| ModelSpec | `packages/expert-work-protocol/src/expert_work/protocol/agent_spec.py:74-148` | `class ModelSpec` |
...
- **fetch_models / 动态发现模型列表不存在**：跟 Hermes 的 `fetch_models()`（每 provider 自己探）不同，Expert Work 不主动列模型 —— 模型名靠 manifest 写死。
...
- **无本地端点自适应 timeout**：跟 Hermes 给本地端点放大 stream_read_timeout 不同，Expert Work 全局用 `stream_deadline_s` 不分本地远程
...
Expert Work 的策略是：**把本地推理当作一种特殊的 OpenAI-compatible provider** ——

- 没有"本地 vs 远程"的概念区分；
- 任何暴露 `/v1/chat/completions` 的服务都一视同仁；
- 唯一特殊点：`base_url` 必填（其他 provider 用默认）。

`packages/expert-work-protocol/tests/test_agent_spec.py:349-358` 是规范用法：
...
| ModelSpec | `packages/expert-work-protocol/src/expert_work/protocol/agent_spec.py:115-120` | `base_url: str | None = None` |
| 测试规范 | `packages/expert-work-protocol/tests/test_agent_spec.py:349-358` | vLLM 配置示例 |
...
- **无 num_ctx 自动探测**：vLLM 模型的实际 context window 写在 `/v1/models` 的 metadata，但 Expert Work 不主动读 —— 必须 manifest 写对 `context_window_size`，写错会导致 L.L2 误判或 token 浪费。
- **无本地 embedding 集成**：J.3 memory 的 embedder 走 qwen `text-embedding-v4` （`/v1/embeddings` 兼容），可以接本地 vLLM embedding 模型，但没有"自动用本地"的优化。
- **无 vLLM-specific 优化**：LoRA adapter selection、prefix caching、speculative decoding 等 vLLM 高级特性都用不上（要靠 vLLM server side 配置，Expert Work 不感知）。
- **无 llama.cpp grammar**：约束输出格式只能靠 LLM 自己（OpenAI function calling），不能用 GBNF。
- **无网络 / 内存检测**：Hermes 有 Tailscale CGNAT 网段识别，Expert Work 没有这种"端点是不是局域网内"的判断。
...
`packages/expert-work-runtime/src/expert_work/runtime/sandbox/runtime_provider.py:1-18` 给出核心拆分：
...
5. **网络硬隔离**：`expert-work-sandbox-egress` 是 Docker `--internal` 网络 —— 沙箱**只能**访问同网络内的 credential-proxy，连不上外网 / 元数据服务 / 宿主。
...
| Runtime provider | `packages/expert-work-runtime/src/expert_work/runtime/sandbox/runtime_provider.py:51+` | `SandboxRuntimeProvider` |
| OCI runtime 字面量 | `runtime_provider.py:27` | `SandboxOciRuntime = Literal["runc", "runsc"]` |
| 默认 egress 网络 | `runtime_provider.py:32` | `DEFAULT_EGRESS_NETWORK = "expert-work-sandbox-egress"` |
...
def docker_run_argv(
    ...
    argv = [
        ...
        *self._workspace_mount(limits, workspace_volume),
        ...
        "--network", self.egress_network,        # expert-work-sandbox-egress
    ]
...
这跟 Hermes 的 docker backend 默认基本一致（cap-drop ALL / no-new-privileges / pids-limit / memory / cpus），但 Expert Work 额外强制 `--read-only` rootfs 和 `--network expert-work-sandbox-egress`，**比 Hermes 默认更紧**。
...
容器内（沙箱内）
  │
  ├─ 没有 Expert Work runtime / 没有 LLM client / 没有 API key
  ├─ rootfs read-only，只能写 /workspace
  ├─ 出站只能到 expert-work-sandbox-egress 网络内的 credential-proxy
...
- **没有 Modal / E2B / Daytona 等 SaaS sandbox**：跟 Hermes 不同 —— Expert Work 设计上**只**支持 Docker + gVisor，避免外部 SaaS 依赖（多租户企业不接受）。
...
> "An agent built at this depth gets **no** ``SubAgentTool`` registered — structural recursion termination, so a cross-manifest cycle (A->B->A) can never run away (Mini-ADR J-12). This replaces a token-budget guard: Expert Work has no runtime token budget, so cost is bounded structurally by depth times each agent's ``max_iterations``."

—— Expert Work 没有运行时 token budget，所以靠"depth × max_iterations" 限。
...
| SubagentStatus 6 态 | `packages/expert-work-protocol/src/expert_work/protocol/subagent.py` | `PENDING/RUNNING/COMPLETED/FAILED/CANCELLED/TIMED_OUT` |
...
- **子 manifest 必须 deploy**：跟 Hermes 的 "spawn arbitrary subagent with custom system_prompt" 不同 —— Expert Work 子 agent 必须是 control-plane 已注册的 AgentSpec，构建期 cycle detection 要能 resolve。
...
- **没有审批自动化分级**：跟 Hermes 的 `_subagent_auto_deny` / `_subagent_auto_approve` 不同，Expert Work 的 sub-agent 直接继承父的审批策略（J.8 PolicySpec），没有"子默认安全"或"子可以 YOLO" 的区分。
...
跟 Hermes 把 cron 当作"独立 daemon 把 prompt 注入新 session"不同，Expert Work 的 trigger 是**完全数据库驱动的状态机**：
...
| 角色 | 文件 | 关键符号 |
|------|------|---------|
| TriggerKind | `packages/expert-work-protocol/src/expert_work/protocol/trigger.py:34` | `Literal["cron", "webhook"]` |
| TriggerSpec / TriggerRecord / TriggerRunRecord | `packages/expert-work-protocol/src/expert_work/protocol/trigger.py:42-143` | DTOs |
...
| TriggerRunStore | `packages/expert-work-persistence/src/expert_work/persistence/trigger_store.py` | SQL + in-memory impl |
| Migration | `packages/expert-work-persistence/migrations/versions/0033_agent_trigger.py` | `agent_trigger` + `trigger_run` 表 |
...
async def receive_webhook(
    trigger_id: UUID,
    request: Request,
    x_expert_work_webhook_secret: str = Header(...),
):
    trigger = await trigger_store.get(trigger_id)
    if not trigger or trigger.kind != "webhook":
        raise HTTPException(404)
    if not hmac.compare_digest(
        hashlib.sha256(x_expert_work_webhook_secret.encode()).hexdigest(),
...
- **Tick 间隔默认偏粗**：不像 Hermes 60s 固定，Expert Work 是 N 秒一轮（具体值看配置），但下限受 DB 查询成本约束。
...
**不在 scope（设计决策）**。这是 Expert Work 与 Hermes **最大的产品定位差异**：Expert Work 不内置任何末端消息平台 adapter（无 Slack / Telegram / 飞书 / 钉钉 / Signal / Discord / ...），由业务系统通过 webhook 接入。

ITERATION-PLAN 锚点：
- `docs/ITERATION-PLAN.md:328`（H Stream 范围澄清原文）：
  > "Business 系统通过 API 消费 Expert Work 的 per-user 持久 agent；Expert Work **不自带末端用户对话 UI**（末端用户通过 business 系统自己的 UI 与 agent 对话）。Admin UI 仅服务操作人群（平台 admin / agent 开发者 / 运营 / SRE）"

## 10.2 设计立场

Expert Work 的产品哲学是 **backend agent platform**：

- **业务系统是 Expert Work 的客户**：客户的产品（聊天软件、SaaS、移动端 app）负责与末端用户交互；
- **Expert Work 提供**：通用 webhook trigger（HMAC 验证）+ SSE stream API + manifest 管理 + Audit / Quota / IAM；
...
| Trigger DTO | `packages/expert-work-protocol/src/expert_work/protocol/trigger.py:34` | `TriggerKind = Literal["cron", "webhook"]` |
...
Expert Work 把"如何让消息进来"的责任完全外推给业务系统：

- **方式 1（推荐，async）**：业务系统接到末端用户消息 → `POST /v1/webhooks/{trigger_id}` + `X-Expert-Work-Webhook-Secret` header → 返回 `run_id` → 用 `GET /v1/sessions/{thread_id}/runs/{run_id}/events` (SSE) 拉响应或轮询 `GET /v1/runs/{run_id}`。
...
- **业务系统必须自己实现 multi-platform adapter**：Slack Block Kit / Telegram InlineKeyboard / 飞书卡片 等富文本组件 Expert Work 不提供。
- **业务系统必须自己实现 rate limit / 反垃圾**：Expert Work 只在 tenant 维度做 quota，不在 platform / chat / user 维度。
- **业务系统必须自己实现媒体上传**：J.6 多模态 upload 端点（`POST /v1/sessions/{thread_id}/uploads`）的 caller 是业务系统，不是末端用户。
- **会话路由（哪个 chat → 哪个 thread）由业务系统决策**：Expert Work 不知道 "telegram:chat_id:12345" 对应哪个 thread，业务系统自己维护这个映射。
- **没有 SessionSource 抽象**：跟 Hermes 的 `SessionSource(platform, chat_id, ...)` 不同，Expert Work 的 thread / session 只有 `tenant_id` + `user_id`，没有 platform 维度。
- **没有 inbound 平台特有的 voice / image / file 协议适配**：业务系统负责把语音转文字、把图片上传到 J.6 endpoint。

> ⚠️ 这个维度的"不在 scope"**不是 backlog**，是 Expert Work 一直会保持的设计选择。M1/M2/M3 都不会内置消息平台 adapter。
...
- **MCP Server（Expert Work 暴露能力给 Claude Code / Cursor）**：❌ **不在 scope**（搜不到任何 `FastMCP` / `MCPServer` / `mcp.server` 引用）。
...
跟 Hermes 双向 MCP（Client + Server）不同，Expert Work 的 MCP 是**单向消费式** —— 只接外部 MCP server 当工具源，不把自己暴露给其他 IDE / agent。
...
| 配置 schema | `packages/expert-work-protocol/src/expert_work/protocol/tenant_config.py` | `TenantConfigPatch.mcp_servers` |
| 迁移 | `packages/expert-work-persistence/migrations/versions/0011_tool_config.py` | `tenant_config.mcp_servers` JSONB |
...
跟 Hermes 配置基本一致，但 **Expert Work 没有 `timeout` / `connect_timeout` / `supports_parallel_tool_calls` / `transport` / `url` / `sampling` 字段**（这些都不在 M0 scope）。
...
> ":class:`MCPTool` — one **Expert Work** :class:`Tool` per MCP-exposed tool, namespaced as ``mcp:<server>.<tool>`` so the LLM sees them as first-class entries in the spec list."
...
- **无 OAuth manager**：跟 Hermes 的 `tools/mcp_oauth.py` 不同，Expert Work 没有 MCP server OAuth 流程支持。
- **无 sampling**：MCP server 不能反向请求 Expert Work 跑一次 LLM 补全。
- **不暴露 MCP Server**：Expert Work 的 tools / sessions / messages 不能被 Claude Code / Cursor 通过 MCP 反向访问。
...
Expert Work 的扩展机制设计哲学完全不同于 Hermes：
...
即 Expert Work M0 故意把"扩展"做得**保守而显式**：不支持 plugin / hook 热加载，所有扩展能力（tool / skill / MCP / sub-agent）都必须通过 manifest 声明 + 数据库存储 + 代码装配。这跟"业务无关多租户企业引擎"的定位一致：**安全审计第一**，agent autonomy 永远是 opt-in。
...
| Skill 解析 | `packages/expert-work-protocol/src/expert_work/protocol/skill.py` | `parse_skill_ref` / `SkillRef` / `SkillStatus` |
...
| Manifest validator | `packages/expert-work-protocol/src/expert_work/protocol/agent_spec.py` | `_check_skills` validator |
...
跟 Hermes 的 `ToolEntry`（含 `check_fn` / `requires_env` / `emoji` / `dynamic_schema_overrides` 等）相比，Expert Work 字段更少 + 更聚焦"调度行为"。
...
—— 8 项里没有"plugin hook 系统"。Expert Work 显然认为这条线（hook）不是产品方向。
...
- **无 supporting files**（M0）：跟 Hermes 的 `references/` / `templates/` / `scripts/` 不同，M0 Expert Work Skill 是单一 prompt fragment + tool name list。M1-K J.7b-6 backlog。
...
- **CLI**：❌ **不在 M0 scope**。M1-I `Expert Work lint` / `Expert Work run`（manifest 本地跑 + lint）。
...
```
Browser → https://expert_work.example.com → nginx (TLS 终止)
                                          │
...
- **无 CLI**：M1-I 才有 `Expert Work lint` / `Expert Work run`；M0 没有本地跑 manifest 工具，全靠 control-plane API。
- **无终端 TUI**：跟 Hermes 的 prompt_toolkit + Rich 不同，Expert Work 完全是 web UI。
...
- **末端用户的 expert-work-side persistence 不可见**：用户在业务系统里看到的不是 Expert Work 的 Admin UI，业务系统自己渲染 Expert Work 返回的 trajectory 给末端用户。
...
- **跟 agentskills.io 标准的对齐**：❌ **不一致**。Expert Work Skill 是 `(prompt_fragment + tool_names + required_models)` 三元组存 Postgres，不是 agentskills.io 的 SKILL.md frontmatter + supporting files。
...
Expert Work 的 Skill 设计**故意不对齐 agentskills.io 标准**，原因是：

1. **多租户隔离强于跨工具可移植性**：每个租户的 skill 严格 per-tenant（`UNIQUE(tenant_id, name)`），不能跨租户引用；agentskills.io 设计上是公共 hub，跨用户跨工具共享，跟 Expert Work 多租户企业定位冲突。
...
| 角色 | 文件 | 关键符号 |
|------|------|---------|
| Skill 协议 | `packages/expert-work-protocol/src/expert_work/protocol/skill.py` | `Skill` / `SkillVersion` / `SkillRef` / `SkillStatus` / `SKILL_REF_PATTERN` / `parse_skill_ref` |
| Skill ORM | `packages/expert-work-persistence/src/expert_work/persistence/models/skill.py:31-66` | `SkillRow` / `SkillVersionRow` |
| Skill Store | `packages/expert-work-persistence/src/expert_work/persistence/skill_store.py` | `SkillStore` ABC + In-Memory + SQL impl |
| 迁移 | `packages/expert-work-persistence/migrations/versions/0029_skill.py` | `skill` + `skill_version` 表 |
...
### 14.4.1 Skill DTO（`packages/expert-work-protocol/src/expert_work/protocol/skill.py`）
...
- **不兼容 agentskills.io 公共 hub**：M0 Expert Work Skill 不能直接 import agentskills.io 上的 SKILL.md。
...
- **Feedback 收集**：M0 ✅（G.6）。
- **轨迹压缩**：❌ 不在 scope（Hermes 有专门的 trajectory_compressor，Expert Work 没有；ShareGPT 直接喂训练流水线）。
...
5. **直接 ShareGPT 格式**：跟 LlamaFactory / Axolotl / TRL 等开源训练框架兼容，不引入 Expert Work 专属格式。
...
| Eval Dataset DTO | `packages/expert-work-protocol/src/expert_work/protocol/eval_dataset.py` | `CurationCandidateRecord` / `EvalDatasetRecord` |
| Curation Worker | `services/control-plane/src/control_plane/curation_worker.py` | `_classify()` |
| Eval Gate | `tools/eval/expert_work_eval.py` | YAML loader + agent runner + baseline 卡 |
...
- **Trajectory compression**：Hermes 有 `trajectory_compressor.py` 用 OpenRouter gemini-3-flash-preview 压缩超 token 的轨迹；Expert Work 没有这一步（ShareGPT 原始 jsonl 直接喂训练流水线）。
- **Reward model**：完全没有；评估靠 user feedback（up/down）+ outcome（completed / failed / max_steps）。
- **训练闭环**：Expert Work 出 ShareGPT JSONL；后续微调（LlamaFactory / Axolotl / TRL / unsloth）由用户自己跑。
...
CI / 手动 eval
  │
  └─ python tools/eval/expert_work_eval.py --baseline m0_gate_baseline.yaml
        ├─ load datasets/*.yaml
...
- **`completed` outcome 是粗粒度**：success 内还有"用户满意 vs 用户最终未满意"差异，Expert Work 用 G.6 feedback 区分但不进 outcome 字段。
...
# 附录 A — Expert Work monorepo 目录速查
...
│
├── packages/                               # 4 个共享 Python SDK
│   ├── expert-work-common/                       # observability / metrics validator
│   ├── expert-work-persistence/                  # SQLAlchemy 2.0 + Alembic
│   │   └── migrations/versions/0001..0038  # 38 个迁移
│   ├── expert-work-protocol/                     # Pydantic DTO 协议层
│   └── expert-work-runtime/                      # middleware / sandbox provider / secret
│
├── services/                               # 7 个独立部署的服务
...
└── docs/
    ...
    └── research/
        ...
        └── expert-work-current-state.md          # 本报告
```
...
| 2. 自我改进 | `services/control-plane/src/control_plane/{curation_worker.py, api/feedback.py}`、`packages/expert-work-protocol/src/expert_work/protocol/eval_dataset.py` |
| 3. 记忆 | `packages/expert-work-persistence/src/expert_work/persistence/memory/{base.py, sql.py, dlq.py}`、`services/orchestrator/src/orchestrator/graph_builder/memory.py`、`migrations/0013/0017/0024/0025` |
| 4. 上下文管理 | `services/orchestrator/src/orchestrator/context/compressor.py`、`packages/expert-work-runtime/.../middleware/{dynamic_context.py, token_usage.py}` |
...
| 7. 沙箱 | `services/sandbox-supervisor/`（整服务 4452 行）、`packages/expert-work-runtime/src/expert_work/runtime/sandbox/runtime_provider.py` |
...
| 9. Cron / Trigger | `packages/expert-work-protocol/.../trigger.py`、`services/control-plane/src/control_plane/{scheduler.py, trigger_firing.py, api/triggers.py}`、`migration 0033` |
| 10. 消息平台 | **不内置**；`services/control-plane/src/control_plane/api/triggers.py:335+` webhook 端点 |
| 11. MCP | `services/orchestrator/src/orchestrator/tools/mcp.py`、`migration 0011`、`packages/expert-work-protocol/.../tenant_config.py` |
| 12. 扩展机制 | `services/orchestrator/src/orchestrator/{tools/registry.py, tools/assembly.py, middleware_assembly.py}`、`packages/expert-work-protocol/.../skill.py` |
| 13. UI | `apps/admin-ui/`（17.5K TS+TSX） |
| 14. 技能可移植 | `packages/expert-work-protocol/.../skill.py`、`packages/expert-work-persistence/src/expert_work/persistence/models/skill.py`、`services/control-plane/src/control_plane/api/skills.py`、`migration 0029` |
| 15. RL / Trajectory | `services/orchestrator/src/orchestrator/trajectory/recorder.py`、`tools/eval/{expert_work_eval.py, export_dataset.py, baselines/m0_gate_baseline.yaml}`、`migration 0034` |
...
| 13. UI - CLI | — | M1-I `Expert Work lint` / `Expert Work run` + JSON Schema | — | — |
| 13. UI - 末端用户 | — | — | — | — |
