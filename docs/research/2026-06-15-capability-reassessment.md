...
| **10.1 连接式 trace** | ★2→★4 | 低估 | A.8：`sse.py:338 expert_work_span(SESSION,"run")` root span + `builder.py:607/1350` LLM/tool 业务 span + `propagation.py` 跨服务传播（`test_sandbox_trace_propagation.py`）；旧 W0「run_agent 无 span」已过期；缺口仅"外部 egress 故意不注"(设计决议) | ✅ 亲核 sse.py |
...
> （record `expert_work_slo_burn_rate{slo,window}` + 长窗确认/短窗加速告警 P0/P1/P2），promtool check+test 双 SUCCESS，
