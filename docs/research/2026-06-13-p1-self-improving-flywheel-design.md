...
目标：一次 run 形成 `helix.session.run` 根 span → 其下 LLM/tool child span → subagent / durable-resume
用 Span Link 关联。10.1 从 ★2 抬到 5★。**Langfuse v3 自动共享 OTel context（`langfuse_sdk.py` 确认，零额外接线）。**

现状（勘探确认）：`helix_span()` 在 `tracing.py:155` 有 `attributes=`、无 `links=`；`HelixComponent`
...
| # | 文件:行 | 改动 |
|---|---|---|
| 1 | `packages/helix-common/.../observability/tracing.py:155` | `helix_span()` 加 `links: Sequence[trace.Link] \| None = None`，透传 `start_as_current_span(links=...)`；其余行为不动 |
| 2 | `services/orchestrator/.../sse.py:229` `run_agent()` | RUNNING 设置(:306)外层包 `helix_span(SESSION, "run", attributes={run_id,agent,agent_version,thread_id})` |
| 3 | `graph_builder/builder.py:547` | LLM `active_caller(...)` 包 `helix_span(ORCHESTRATOR, "llm_call", {step,model})` |
| 4 | `graph_builder/builder.py:1116` | `_invoke_tool` 包 `helix_span(ORCHESTRATOR, "tool_call", {tool})` |
| 5 | `tools/subagent.py:222` + `_child_config:461` | 取当前 span context 作 Link 传 `child_config[...]["parent_span_link"]`；child root span 经 #1 `links=` 关联回 parent（child 用新 trace，合设计 §5.5） |
| 6 | `run_retry.py` / resume 路径 | resume 时 root span 加指向上次 checkpoint span 的 Link（无 id 则 attribute 占位，不阻断） |

### 复用 / 不重造
- `helix_span` / `HelixComponent`（枚举已全）；现有 metrics 打点（sse.py:102）不动，trace 与 metrics 并存。
- Langfuse 自动共享（v3 OTel-based）。
- 测试范式：`packages/helix-common/tests/test_observability_tracing.py` 的 `InMemorySpanExporter` + `get_finished_spans()`。

### 验证
- 单测：`helix_span(links=...)` 透传 Link。
- 集成测：minimal graph 跑 run_agent → 断言 finished spans 含 1 root `helix.session.run` + N `llm_call` + M `tool_call` 挂 root；subagent 场景断言 child root span 含指向 parent 的 Link。
- 回归：`uv run python -m pytest packages/helix-common/tests/test_observability_tracing.py services/orchestrator/tests/ -m "not integration"`。
