...
`langfuse-web` 用 `LANGFUSE_INIT_*` 自动建好 org `helix` + project `helix-dev` +
固定 key，无需点 UI：
...
写进 `.env`（或 shell export）：

```bash
# control-plane 在 compose 内 → 用服务名;在宿主跑 → 用 localhost:3001
HELIX_CONTROL_PLANE_LANGFUSE_HOST=http://langfuse-web:3000
HELIX_CONTROL_PLANE_LANGFUSE_PUBLIC_KEY=pk-lf-helix-dev
HELIX_CONTROL_PLANE_LANGFUSE_SECRET_KEY=sk-lf-helix-dev
# PII 脱敏默认开;仅应急排查原始 prompt 时显式关:
# HELIX_CONTROL_PLANE_LANGFUSE_PII_MASKING_ENABLED=false
```
...
2. Langfuse UI → project `helix-dev` → Traces，应出现一条 generation，
