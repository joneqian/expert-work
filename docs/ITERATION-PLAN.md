...
- [ ] **OA-3b-2 build 路径接入**：把 `subject_id`(=连接 user_id,**非** caller_user_id)串过 get_agent→builder→build_agent→子 agent + `ToolEnv.user_mcp_oauth_pool` + assembly 注册 + 缓存 key 加 user 维 + callback invalidate user pool + e2e
