...
- **SE-7 全自动护栏**：拆 SE-7a（auto-promote 策略）+ SE-7b（速率限制 + 熔断）+ SE-7c（回归回滚 + set_status 接线 + 审计/指标）
  - [x] **SE-7a auto-promote 策略**（PR #TBD）：`control_plane/skill_promotion.py` `decide_promotion` 纯决策——grounded DRAFT → `AUTO_PROMOTE`/`HUMAN_REVIEW`/`HOLD`。优先级(硬到软):未 grounded→HOLD；**高危→永远人审**(SE-A0)；熔断开→人审(降级全人审,SE-A12)；不 eligible(SE-4a 信号层级/锚点)→人审；超速率→人审(SE-A12)；否则自动。`should_auto_promote` 便捷判定。9 单测 + ruff/mypy 绿（SE-A10）
