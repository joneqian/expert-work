...
- **SE-6 进化 worker（Layer B 引擎）**：拆 SE-6a（co-evolve 编排核心）+ SE-6b（worker 外壳 + 真实接线 + lifespan）
  ...
  - [x] **SE-6b worker 外壳**（PR #TBD）：`control_plane/skill_evolution_worker.py` `SkillEvolutionWorker`——克隆 CurationWorker 骨架(start/stop/_loop/run_once + `_bypass_rls` 跨租户扫 + `_tenant_scope` 逐租户)。`run_once` 扫 PENDING candidate、过滤 `signal∈{positive_feedback,failed_outcome}`、batch 截断、逐个委托注入的 `CandidateProcessor`、按 outcome 计 `EvolutionTally`。可观测:cycle_errors + grounded counter。per-candidate 重活(组装轨迹/接真 aux LLM/graph replay/落 DRAFT)经 `processor` 注入,CI fake 全测。6 单测 + ruff/mypy 绿（SE-A9）
  - [ ] **SE-6c 真 processor + lifespan**：实现 `CandidateProcessor`——组装成功/失败轨迹 + 锚点重放集、接真 aux LLM(distiller/attributor)+ GraphReplayTaskRunner + ReplayRunner、co-evolve 各轮持久化 DRAFT(evolution_origin='distilled'+溯源)、交治理门(SE-7);wire app lifespan;integration run_once 全链（SE-A9）
