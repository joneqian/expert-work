...
- [ ] **RT-5 生产质量监控**（~2 周，5 PR，★5 需 live E2E）（设计 [STREAM-RT-DESIGN](./streams/STREAM-RT-DESIGN.md) §13 / RT-ADR-22~26）：**grounding 核实计划三处"复用"假设过期**——① `eval_engine_live` 跑合成数据集 + 确定性判分（safety_verdict/evaluate_trace），不采真流量、不调 LLM judge（仅脚手架可复用）；② `_judge.py` 直连 Anthropic + 正则抠单 int，**绕开 router、未接 RT-1**，RT-1 结构化输出在 router 侧（`router.py:384` output_schema + `structured_output.py`，消费方=aux_model_adapter/consolidator/distiller，**不含 _judge**）→ 判分走 RT-1-correct 的 router `output_schema` 路径不复用 _judge；③ webhook 全挂 run_event spine（`event_id={run_id}:{seq}`），`quality.drift` 非 run 事件 → 需新 emitter（run_id=None、自造 event_id），payload_format 渲染正交可复用。**决策**（产品意图不变，只改复用谁）：pull 式 `QualityMonitorWorker`+水位+确定性采样（脱 run 热路径，采样率+日上限护栏）→ `QualityJudge`（RT-1 router output_schema + 通用 rubric，不碰 _judge，surgical 独立 caller）→ 新 `quality_score` 时序表（RLS）→ `QualityDriftWorker` 滑窗基线偏离 + `quality_drift_alert` cooldown → `quality.drift` 新 emitter（4 同步点）；aux 计量 `usage_kind="quality_sampling"`（memory_consolidation 对称）；质量看板新页 + 平台/租户质量配置。诚实边界：抽样非全量／LLM 主观分非 ground-truth／漂移是统计信号不自动处置（`feedback_no_design_choice_disguise`）。
  ...
  - [ ] **PR-1 后端 采样+判分+落库+计量**：质量配置（平台默认+租户覆盖）+ `QualityMonitorWorker`（pull+水位+确定性采样+日上限）+ `QualityJudge`（RT-1 router output_schema + `QualityScore` rubric）+ `quality_score` 表 + migration 0117 + aux 计量 `usage_kind` + 单测/真 PG+RLS。
