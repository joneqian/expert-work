...
- **SE-7 全自动护栏**：拆 SE-7a（auto-promote 策略）+ SE-7b（速率限制 + 熔断）+ SE-7c（回归回滚 + set_status 接线 + 审计/指标）
  ...
  - [x] **SE-7c promote 接线 + 审计**（PR #TBD）：`control_plane/skill_promotion_gate.py` `PromotionGate`——查 `decide_promotion`(7a)+ `RateLimiter`/`CircuitBreaker`(7b)→ `AUTO_PROMOTE` 则 `set_status(ACTIVE)` + rate.record + emit `SKILL_EVOLUTION_AUTO_PROMOTED`(新 AuditAction,protocol-only StrEnum);否则留 DRAFT。`auto_promote_eligible` 从 SE-4a decide_grounding 一路 thread:ReplayOutcome→EvolutionResult(SE-6a)→invoker(SE-6d)→processor(SE-6c 接 gate,clock 注入)。`build_evolution_worker` 装配 gate(rate 5/h、breaker 0.5/min5/24h)+ app lifespan 传 audit。9(gate)+ 1(processor wiring)单测 + 69 全 SE 回归 + 532 mypy 绿（SE-A10/A12）
