...
- **SE-7 全自动护栏**：拆 SE-7a（auto-promote 策略）+ SE-7b（速率限制 + 熔断）+ SE-7c（promote 接线 + 审计）+ SE-7d（回归回滚）
  ...
  - **SE-7d 回归回滚**：拆 SE-7d-1（归因采集）+ SE-7d-2（判定器）+ SE-7d-3（monitor 接线）。按**专表 `skill_run_usage`**(§4.4) skill-centric 归因（**不借 trajectory metadata**——查询 skill-centric vs trajectory run-centric，建模错配）；**按 skill_version 判定不连坐**（SE-A11）
    - [ ] **SE-7d-1 归因采集**：迁移 `skill_run_usage` 表(tenant_id NULL-RLS + `(tenant_id,skill_id,skill_version,created_at)` 索引) + `SkillRunUsage` DTO + SkillStore 写/窗口聚合方法(base+sql+memory)。`_load_skills` 绑定时已知 skill+version，run 收尾 best-effort 落一行(run 级一次、非 per-step、失败 swallow，同 `SkillActivityRecorder` 纪律；覆盖 build-time + runtime 两种绑定）
