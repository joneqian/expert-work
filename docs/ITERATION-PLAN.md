...
### Wave 2（并行，W1 收完启动）
- [ ] **RT-3 prompt cache 成本工程**（设计 [STREAM-RT-DESIGN](./streams/STREAM-RT-DESIGN.md) §10 / RT-ADR-13~15）：**grounding 核实计划假设过期——后端基本已建好**（四段计量端到端 + Anthropic cache_control 断点编排 + cache 折扣定价列全已存在）。真实 gap = **前端透出**（SettingsUsage cost/token/kind 三表只 input/output/billed，cache 仅 2 headline tile；Chargeback 零 cache）。收敛为 **PR-0 设计 / PR-1 前端透出**（cache 列 + 命中率前端派生 + 节省估算 + i18n，预期零后端）。**keep-warm 不做**（RT-ADR-13，多租户成本不划算，用户拍板）；tool-def 断点推迟（RT-ADR-15）。OpenAI 无 cache_control 是正确的（隐式缓存读 cached_tokens 即可，非缺陷）。
- [ ] **RT-4 全局 kill switch**（设计 [STREAM-RT-DESIGN](./streams/STREAM-RT-DESIGN.md) §11 / RT-ADR-16~18）：grounding 核实——**租户级 suspend 已有**（TenantStatus + TenantStatusService + 中间件/准入 403，但只拒新不杀运行中）；**agent 级 disable 缺**；**bulk-cancel 运行中 run 缺**（RunManager.cancel 只按 run_id）。决策（用户拍板）：agent 级 disable（净新，独立 `agent_disable` 表 + 缓存服务 + 三 gate + enable，RT-ADR-16）+ **bulk-cancel 运行中 run 两级都做**（RT-ADR-17，新 `list_running_runs(scope)` + 循环 RunManager.cancel，跨副本经 abort_event；agent disable + 租户 suspend 都补终止）+ 审计（RT-ADR-18，新 AuditAction `agent:disabled/enabled` + resource_type `agent` 加两处 Literal）。PR-0 设计 / PR-1 后端（表+migration+service+三 gate+bulk-cancel+API+审计+单测）/ PR-2 前端（AgentDetail danger 按钮复用 SettingsTenants Popconfirm 模式 + 状态标识 + i18n）。
  ...
  - [ ] **PR-2 前端**（本次）：AgentDetail disable/enable danger 按钮（复用 SettingsTenants Popconfirm）+ 状态标识 tag + i18n 双语。
