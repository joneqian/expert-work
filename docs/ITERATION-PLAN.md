...
- **SE-6 进化 worker（Layer B 引擎）**：拆 SE-6a（co-evolve 编排核心）+ SE-6b（worker 外壳 + 真实接线 + lifespan）
  ...
  - [x] **SE-6d 真 invoker + providers + lifespan**（PR #TBD）：`control_plane/skill_evolution_assembly.py`(纯助手:信号层级选择/golden prompt 提取/轨迹首消息,**CI 测**)+ `skill_evolution_wiring.py`(真 `_GraphReplayInvoker`[ReplayRunner+GraphReplayTaskRunner,per-candidate `agent_spec_store` 取 active spec + `make_agent_builder`]+ `_TrajectoryEvidenceProvider`[渲染成功/失败轨迹]+ `_DualSourceHeldOutProvider`[golden 优先含 expected→assertion 锚点 + 同 agent success 轨迹兜底]+ `_AuxText`/`_AuxJudge` aux LLM 适配 + `build_evolution_worker`,**lazy 引 orchestrator 避 cycle**)+ app lifespan gated 接线(`enable_skill_evolution_worker` 默认 OFF,自建 aux_model,start/stop)。**验证边界(诚实)**:真 graph/LLM 路径 CI(pytest+integration)均无 model key 跑不了 → 归 SE-9 基准/手动;持久化/RLS 路径由 SE-1/SE-2 SQL store integration + SE-6c 胶水覆盖。6 纯助手单测(合计 44 SE 单测)+ ruff/mypy 绿 → **SE-6 Layer B 引擎完整(6a 脑/6b 壳/6c 胶水/6d 真接线)**（SE-A9）
