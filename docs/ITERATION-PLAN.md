...
- **SE-7 全自动护栏**：拆 SE-7a（auto-promote 策略）+ SE-7b（速率限制 + 熔断）+ SE-7c（回归回滚 + set_status 接线 + 审计/指标）
  ...
  - [x] **SE-7b 速率限制 + 熔断**（PR #TBD）：`control_plane/skill_evolution_limits.py` `RateLimiter`(per-key 滑动窗 max/window → `within_limit`→SE-7a `within_rate_limit`)+ `CircuitBreaker`(窗内失败率超阈且样本≥min → `is_open`→SE-7a `breaker_open`,降级全人审)。纯逻辑、注入 `now` 保确定;in-process 状态(单 worker 够,分布式后续)。9 单测 + ruff/mypy 绿（SE-A12）
