...
Expert Work 服务多业务线（医疗 + HR + 客服 + 研发等），不同业务的合规要求差异巨大：医疗租户要 HIPAA、HR 要 GDPR、金融要 SOX、研发工具几乎没有合规要求。把合规硬编码成"全引擎默认开启"会让非合规租户付不必要的性能/成本代价；硬编码"默认不开"又会让合规租户漏出 PII。
...
| 团队 LangGraph 不熟 | 内部培训 + Expert Work SDK 屏蔽 LangGraph 概念 |
