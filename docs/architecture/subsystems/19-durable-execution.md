...
外存路径命名：`s3://helix-state/{tenant}/{thread_id}/{checkpoint_id}/{field}.json`，由 [22 DR](./22-disaster-recovery.md) 接管备份。
...
helix_checkpoint_size_bytes{tenant}                           histogram
helix_thread_state{tenant,state}                              gauge
helix_thread_pause_duration_seconds{tenant,reason}            histogram
helix_idempotent_replay_total{tenant,tool,result}             counter
   # result = hit_ok | hit_failed | miss
helix_resume_total{tenant,outcome}                            counter
   # outcome = ok | version_mismatch | force | rejected
helix_state_blob_externalize_total{tenant,field}              counter
```

### 7.2 OTel span

- `helix.durable.checkpoint`（attrs：thread_id, size_bytes, event_count, duration_ms）
- `helix.durable.replay`（attrs：thread_id, replayed_events, hit_count）
- `helix.durable.pause`（attrs：thread_id, reason）
- `helix.durable.resume`（attrs：thread_id, actor_id, force）
